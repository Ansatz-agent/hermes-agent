...
test('forwards the selected branch to the runtime checker and preserves its status', async () => {
  const status = { supported: true, updateAvailable: false, reason: 'unpublished-source', message: 'Publish the commit first.' }
  ...
  expect(vi.mocked(execFile).mock.calls[0][1]).toEqual([
    '-m', 'hermes_cli.bundled_update', '--root', '/fixture', '--branch', 'release'
  ])
})
