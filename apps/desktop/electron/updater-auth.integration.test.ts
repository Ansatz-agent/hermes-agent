...
for (const state of ['authenticated', 'signed-out']) {
  test(`detached updater reaches the Desktop auth owner and preserves ${state} authorization`, async () => {
    ...
    const owner = spawn(python, ['-u', path.join(repoRoot, 'tests/fixtures/desktop_update_auth_owner.py'), state], {
      cwd: repoRoot,
      env: ansatzAuthEnvironment(home, environment),
      stdio: 'pipe',
      windowsHide: true
    })
    ...
    try {
      const ready = await Promise.race([
        ...
        ownerExited.then(() => {
          throw new Error(ownerErrors || 'auth owner exited before readiness')
        })
      ])
      ...
      const after = await output(
        spawnUpdaterProcess(python, args, {
          cwd: repoRoot,
          env: environment,
          detached: true,
          stdio: 'pipe'
