...
function artifactName(osName, extension) {
  return PACKAGE_JSON.build.artifactName
...
const DEFAULT_HERMES_HOME = (() => {
  ...
  return path.join(os.homedir(), '.ansatz-voice-trace-client')
})()
...
function resolveDmgPath() {
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, artifactName('mac', 'dmg'))
}

function resolveNsisPath() {
  // Match the product-owned electron-builder artifact template.
  if (!exists(RELEASE_ROOT)) return null
  const candidates = fs
    .readdirSync(RELEASE_ROOT)
    .filter(name => name === artifactName('win', 'exe'))
    .sort((a, b) => {
      const aMtime = fs.statSync(path.join(RELEASE_ROOT, a)).mtimeMs
      const bMtime = fs.statSync(path.join(RELEASE_ROOT, b)).mtimeMs
      return bMtime - aMtime
