...
export const ja = defineLocale({
  auth: {
    ...
    bootstrap: {
      ...
      overallProgressLabel: 'Ansatz ランタイムのインストール',
      authPreparationFailed: '安全なサインインサービスを準備できませんでした。',
      runtimePreparationFailed: 'Ansatz のローカルランタイムをインストールできませんでした。',
  ...
  notifications: {
    ...
    errors: {
      ...
      methodNotAllowed:
        'デスクトップバックエンドがそのリクエストを拒否しました (405 Method Not Allowed)。Ansatz を再起動してください。',
  ...
  settings: {
    ...
    gateway: {
      ...
      intro:
        'Ansatz はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している Hermes バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。以下でプロファイルを選択して、それぞれのリモートホストを設定します。',
      appliesTo: '適用対象',
      ...
      restartingMessage: 'Ansatz は保存された設定を使用して再接続します。',
      ...
      sshErrPlatform:
        'サポートされていないリモートプラットフォームです。Ansatz の SSH モードは Linux、macOS、Windows のリモートホストに対応しています。',
  ...
  skills: {
    ...
    skillArchivedMessage: 'ansatz curator restore で復元できます。'
  },
  ...
  install: {
    ...
    setupChoiceTitle: 'Ansatz をセットアップ',
    setupChoiceDesc:
  ...
  desktop: {
    ...
    restartToUseSaveImage: '画像を保存するには Ansatz を再起動してください。',
    restartToSaveImages: '画像を保存するには Ansatz を再起動してください',
    ...
    handoff: {
      ...
      timedOut: 'ゲートウェイの待機がタイムアウトしました。`ansatz gateway` は起動していますか？'
    }
  },
