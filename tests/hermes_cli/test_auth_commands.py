...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    # `hermes provider add nous` must also populate providers.nous so the
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`hermes provider add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``hermes provider add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``hermes provider list`` showed two labels sharing
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """hermes provider add xai-oauth must set active_provider and write a pool entry.
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``hermes provider add xai-oauth`` runs must produce independent pool entries.
