...
VOICE_REFERENCE_BLOBS = {
    ...
    "tools/sensevoice_stt.py": "4ebd81f113e80fd6af8d57a9b4c560b1cd1f4c18",
}
...
def test_desktop_package_keeps_both_platform_build_commands_and_safe_inputs() -> None:
    ...
    assert scripts["dist:mac:dmg"] == "npm run build && npm run builder -- --mac dmg"
    assert scripts["dist:win:nsis"] == "npm run build && npm run builder -- --win nsis"
...
def test_sensevoice_download_is_modelscope_first_and_hash_pinned() -> None:
    ...
    assert urls[1].startswith("https://github.com/k2-fsa/sherpa-onnx/releases/download/")
