...
Older versions of ``install.sh`` could leave a command path as a symlink to a
pip-generated entry point. When ``setup_path()`` writes a canonical bash shim
with ``cat > "$command_link_dir/ansatz" <<EOF``, the redirect must not follow
an existing symlink and overwrite the pip entry point with the shim. The
resulting launcher would then self-recurse and ``ansatz`` would hang on
every invocation.

These tests pin the fix: ``setup_path()`` must remove ``$command_link_dir/ansatz``
...
def _extract_setup_path_shim_block() -> str:
    ...
    match = re.search(
        r"(?P<block>    rm -f \"\$command_link_dir/ansatz\".*?chmod \+x \"\$command_link_dir/ansatz\")",
        text,
        re.DOTALL,
    )
...
def test_setup_path_shim_block_removes_old_link_before_writing() -> None:
    ...
    rm_idx = block.find('rm -f "$command_link_dir/ansatz"')
    cat_idx = block.find('cat > "$command_link_dir/ansatz" <<EOF')
    assert rm_idx != -1, (
        "setup_path() must `rm -f` $command_link_dir/ansatz before the "
...
def test_re_running_setup_path_block_preserves_pip_entry_point(tmp_path: Path) -> None:
    ...
    Layout mirrors a real install:
        ...
        tmp/
          venv/bin/ansatz        <- pip entry point (the one we must preserve)
          local_bin/ansatz       <- symlink → ../venv/bin/ansatz  (old install)
    ...
    requires that, after the run:
          ...
          * ``venv/bin/ansatz`` still contains its original pip-script body
          * ``local_bin/ansatz`` is a regular file holding the shim
    """
    venv_bin = tmp_path / "venv" / "bin"
    venv_bin.mkdir(parents=True)
    pip_entry = venv_bin / "ansatz"
    ...
    shim_path = command_link_dir / "ansatz"
    ...
    script = (
        "set -e\n"
        f"quoted_ansatz_bin={pip_entry!s}\n"
        f"quoted_ansatz_entrypoint={pip_entry!s}\n"
    ...
    assert pip_entry.read_text() == pip_marker, (
        "venv/bin/ansatz was overwritten by setup_path() — symlink-stomp "
        "regression (#21454)."
    )
    ...
    assert not shim_path.is_symlink(), (
        "command_link_dir/ansatz must be replaced with a regular file, not "
    ...
    assert str(pip_entry) in shim_text
