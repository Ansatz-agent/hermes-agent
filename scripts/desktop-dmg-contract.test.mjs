...
test('browser download validator accepts ordinary Electron build output', () => {
  const log = [
    ...
    'building block map',
    'Hermes-0.17.0-mac-arm64.dmg'
  ].join('\n')

  assert.equal(forbiddenBrowserDownloadLine(log), null)
})

test('findNewestDmg returns the newest macOS arm64 Hermes artifact', () => {
  const releaseDir = fs.mkdtempSync(path.join(os.tmpdir(), 'hermes-dmg-release-'))
  try {
    const older = path.join(releaseDir, 'Hermes-0.16.0-mac-arm64.dmg')
    const newer = path.join(releaseDir, 'Hermes-0.17.0-mac-arm64.dmg')
    fs.writeFileSync(older, 'older')
    fs.writeFileSync(newer, 'newer')
    fs.writeFileSync(path.join(releaseDir, 'Hermes-0.17.0-mac-x64.dmg'), 'wrong arch')
...
test('findNewestDmg fails when the build produced no matching artifact', () => {
  const releaseDir = fs.mkdtempSync(path.join(os.tmpdir(), 'hermes-dmg-release-'))
  try {
    assert.throws(() => findNewestDmg(releaseDir), /no macOS arm64 Hermes DMG found/)
  } finally {
    fs.rmSync(releaseDir, { recursive: true, force: true })
  }
})
