...
export function findNewestDmg(releaseDir, notBeforeMs = 0) {
  ...
  const candidates = fs.existsSync(absoluteReleaseDir)
    ? fs.readdirSync(absoluteReleaseDir)
      .filter(name => /^Ansatz-.+-mac-arm64\.dmg$/.test(name))
  ...
  if (candidates.length === 0) {
    throw new Error(`no macOS arm64 Ansatz DMG found in ${absoluteReleaseDir}`)
  }
  return candidates[0]
}
...
if (invokedPath === import.meta.url) {
  try {
    ...
    process.stderr.write(`Ansatz DMG build: ${error.message}\n`)
    process.exitCode = 1
  }
}
