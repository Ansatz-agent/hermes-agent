...
Set via `ansatz model` (picker) or `ansatz setup`. 35+ provider profiles ship as
...
| nous | OAuth device code | `ansatz auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `ansatz auth add openai-codex` |
| qwen-oauth | OAuth | `ansatz auth add qwen-oauth` |
| minimax-oauth | OAuth | `ansatz auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`ansatz auth`).
Fallback chain when the primary fails: `ansatz fallback add|remove|list`.
...
#   ansatz config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
