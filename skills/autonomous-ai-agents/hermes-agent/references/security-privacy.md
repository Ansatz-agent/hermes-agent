...
```bash
ansatz config set security.redact_secrets true       # keep enabled globally
```
...
```bash
ansatz config set privacy.redact_pii true    # enable
ansatz config set privacy.redact_pii false   # disable (default)
```
...
```bash
ansatz config set approvals.mode smart       # recommended middle ground
ansatz config set approvals.mode off         # bypass everything (not recommended)
```
...
1. Shell-command allowlist: `ansatz config set command_allowlist '[]'`
2. Shell-hook consent (only if present): `rm -f ~/.hermes/shell-hooks-allowlist.json`

Then sanity-check `ansatz config get approvals.mode` (should not be `off`)
...
To keep the model away from network or media tools entirely, open `ansatz tools` and toggle per-platform. Takes effect on next session (`/reset`). See `references/configuration.md` for the toolset list.
