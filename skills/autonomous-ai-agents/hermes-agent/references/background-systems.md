...
the `cronjob` tool, the `ansatz cron` CLI (`list`, `add`, `edit`,
...
- **CLI:** `ansatz curator <verb>` — `status`, `usage`, `run`, `pause`,
...
- **Cost:** the deterministic inactivity/prune sweep runs for free. The
  ...
  **off by default** — opt in with `curator.consolidate: true` or
  `ansatz curator run --consolidate`. Routine background curation costs
  zero tokens.
...
Users drive it via `ansatz kanban <verb>`; dispatcher-spawned workers
