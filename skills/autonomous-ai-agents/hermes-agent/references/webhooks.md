...
All management is via the `ansatz webhook` CLI command:

### Create a subscription
```bash
ansatz webhook subscribe <name> \
...
```bash
ansatz webhook subscribe todoist-hermes \
...
```bash
ansatz webhook remove <name>
```

### Test a subscription
```bash
ansatz webhook test <name>
ansatz webhook test <name> --payload '{"key": "value"}'
```
...
### GitHub: PR reviews
```bash
ansatz webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
ansatz webhook subscribe stripe-payments \
...
```bash
ansatz webhook subscribe ci-builds \
  --events "pipeline" \
...
```bash
ansatz webhook subscribe alerts \
...
1. `ansatz webhook subscribe` writes to `~/.hermes/webhook_subscriptions.json`
...
4. **Signature mismatch?** Verify the secret in your service matches the one from `ansatz webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `ansatz webhook test <name>` to verify the route works.
