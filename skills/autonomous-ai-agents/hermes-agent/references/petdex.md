...
the Hermes CLI, TUI, and desktop app. This skill drives the `ansatz pets` CLI
...
Use the `terminal` tool to run `ansatz pets <subcommand>`.

## Quick Reference

| Goal | Command |
| --- | --- |
| Browse the gallery | `ansatz pets list` (add a substring to filter: `ansatz pets list cat`) |
| List installed pets | `ansatz pets list --installed` |
| Install a pet | `ansatz pets install <slug>` (add `--select` to make it active) |
...
| Preview/animate in terminal | `ansatz pets show [slug] [--cycle] [--state run]` |
| Disable the pet | `ansatz pets off` |
| Remove a pet | `ansatz pets remove <slug>` |
| Diagnose setup | `ansatz pets doctor` |

## Procedure

1. Find a pet: `ansatz pets list <query>` and note its `slug`.
2. Install + activate: `ansatz pets install <slug> --select`.
3. Preview it: `ansatz pets show` (Ctrl+C to stop).
4. Confirm setup: `ansatz pets doctor` — shows the resolved pet, configured
...
- `scale` (float) — on-screen size of the native 192×208 frames (default 0.33,
  ...
  `ansatz pets scale <factor>`, the `/pet scale` slash command, or the desktop
  Appearance slider.
...
- `ansatz pets doctor` reports `✓ ready` when a pet is installed, selected,
