...
Edit with `ansatz config edit` or `ansatz config set section.key value`.
...
`ansatz config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `ansatz tools` (interactive) or `ansatz tools enable/disable NAME`.
