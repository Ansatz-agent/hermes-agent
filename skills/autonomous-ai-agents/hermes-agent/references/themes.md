...
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `ansatz config set`).
...
3. **Apply it yourself — never hand-edit `config.yaml`.** Run the safe writer via
   `terminal`:
   ```
   ansatz config set display.skin <name>
   ```
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `ansatz config set display.skin default` (or they can `/skin default`).
...
```
ansatz skin set <key> <hex>      # e.g. ansatz skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `ansatz skin set background "#08201f"` changes
only the background; `ansatz skin use <name>` / `ansatz skin list` switch and
enumerate.
...
- **Never hand-edit `config.yaml` to activate.** Use `ansatz config set
...
- **You apply it, not the user.** `ansatz config set display.skin <name>` is
...
- Run `ansatz config get display.skin` and confirm it reports `<name>`.
