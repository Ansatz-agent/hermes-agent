"""ansatz hooks — inspect and manage shell-script hooks.

Usage::

    ansatz hooks list
    ansatz hooks test <event> [--for-tool X] [--payload-file F]
    ansatz hooks revoke <command>
    ansatz hooks doctor
...
def hooks_command(args) -> None:
    """Entry point for ``ansatz hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: ansatz hooks {list|test|revoke|doctor}")
        print("Run 'ansatz hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs and not outbound:
        ...
        print("See `ansatz hooks --help` or")
                    ...
                    if entry and entry.get("approved_at"):
                        ...
                        if mtime_now and mtime_at and mtime_now > mtime_at:
                            print(
                                ...
                                f"run `ansatz hooks doctor` to re-validate"
                            )
            print()
...
# stdin a script sees under `ansatz hooks test` and `ansatz hooks doctor`
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `ansatz hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `ansatz hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `ansatz hooks doctor`.")
