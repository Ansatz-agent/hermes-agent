"""Implementation of the ``ansatz bundles`` CLI subcommand.
...
def _console() -> Console:
    # Bind to stderr so piping `ansatz bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  ansatz bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``ansatz bundles`` argparse tree.
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `ansatz bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``ansatz bundles <subcommand>`` to the right handler."""
