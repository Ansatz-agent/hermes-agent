"""CLI subcommand: ``ansatz send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"ansatz send: {file_path} is not a text file. --file reads the "
                ...
                f'  ansatz send --to telegram "MEDIA:{file_path}"\n'
                f'  ansatz send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  ansatz send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"ansatz send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"ansatz send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"ansatz send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"ansatz send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    # setup used only for outbound `ansatz send`) would otherwise be
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"ansatz send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("Set one up with `ansatz gateway setup`, or run the gateway once so")
...
def _load_hermes_env() -> None:
    ...
    startup that ``ansatz send`` must replicate when invoked standalone:
    ...
    2. Bridge top-level simple values from ``~/.hermes/config.yaml`` into
       ...
       via ``ansatz config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``ansatz send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    if not target:
        print(
            "ansatz send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  ansatz send --to telegram \"hello\"\n"
            "  ansatz send --to discord:#ops --file report.md\n"
            "  ansatz send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "ansatz send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `ansatz send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        epilog=(
            ...
            "  ansatz send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `ansatz send --list telegram`.",
    )
