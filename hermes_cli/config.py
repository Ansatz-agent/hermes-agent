...
- ansatz config          - Show current configuration
- ansatz config edit     - Open config in editor
- ansatz config get      - Print a resolved configuration value
- ansatz config set      - Set a specific value
- ansatz config unset    - Remove a user configuration value
- ansatz config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``ansatz config set`` (which rewrites
...
def _warn_config_parse_failure(
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``ansatz logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  ansatz config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   on every plugin install / ``ansatz update``.
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `ansatz plugins enable
    # observability/langfuse` or `ansatz tools → Langfuse`); credentials gate
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    ``docker`` and ``ansatz update`` refuses to run ("doesn't apply inside the
    ...
    #    container, and honouring it wrongly blocks ``ansatz update``.
    try:
        method = (
...
# Long-form text for ``ansatz update`` / ``--check`` when running inside the
...
✗ ``ansatz update`` doesn't apply inside the Docker container.
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``ansatz update`` inside Docker.
...
Ansatz was installed from a Desktop source payload.
Run `ansatz update` to fetch published updates from Ansatz-agent/hermes-agent.
The first update prepares a Git checkout and preserves the previous source in a backup."""
...
def ensure_hermes_home():
    ...
    if key in _HERMES_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``ansatz profile create``).
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `ansatz config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # should never break `ansatz update`.  Skill-config prompting is a
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'ansatz setup' to create a valid config")]
    ...
    # scalars are bridged into os.environ (gateway/run.py, ansatz send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'ansatz doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "ansatz update / ansatz -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ ansatz config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: ansatz config set <key> <value>")
    ...
    # `ansatz update`, but only the version bump is persisted.
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``ansatz config set``
    ...
    backends) — while display paths (``ansatz status``/``dump``) read ``name``
...
#   openai-codex (OAuth — ansatz auth) — OpenAI Codex
#   nous         (OAuth — ansatz auth) — Nous Portal
...
def load_env() -> Dict[str, str]:
    ...
    interactive menu render (`ansatz tools`, `ansatz setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `ansatz tools`
...
def show_config():
    ...
    try:
        ...
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                ...
                f"(run 'ansatz doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  ansatz config edit     # Edit config file", Colors.DIM))
    print(color("  ansatz config set <key> <value>", Colors.DIM))
    print(color("  ansatz setup           # Run setup wizard", Colors.DIM))
    print()
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `ansatz cron list`, then pin the "
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # `ansatz config set plugins.index_url ...` validation.
    "plugins",
})
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``ansatz config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``ansatz config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``ansatz config set --force``.
    """
    if is_managed():
    ...
    if config_path.exists():
        try:
            ...
            print(
                ...
                f"  (ansatz config edit will open it in your editor.)",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    # ``ansatz config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``ansatz config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    ansatz config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    ansatz config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    _set_nested(user_config, key, value)
    # Normalize the api_base → base_url alias at set-time too (issue #8919),
    # so a fresh `ansatz config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `ansatz config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `ansatz config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: ansatz config get <key> [--json]")
            print()
            print("Examples:")
            print("  ansatz config get model")
            print("  ansatz config get terminal.backend")
            print("  ansatz config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: ansatz config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  ansatz config set model anthropic/claude-sonnet-4")
            print("  ansatz config set terminal.backend docker")
            print("  ansatz config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: ansatz config unset <key>")
            print()
            print("Examples:")
            print("  ansatz config unset model")
            print("  ansatz config unset terminal.backend")
            print("  ansatz config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'ansatz config migrate' to add them")
        ...
        print("  ansatz config           Show current configuration")
        print("  ansatz config edit      Open config in editor")
        print("  ansatz config get <key>          Print a resolved config value")
        print("  ansatz config set <key> <value>   Set a config value")
        print("  ansatz config unset <key>        Remove a config value")
        print("  ansatz config check     Check for missing/outdated config")
        print("  ansatz config migrate   Update config with new options")
        print("  ansatz config path      Show config file path")
        print("  ansatz config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``ansatz config`` UI so users
