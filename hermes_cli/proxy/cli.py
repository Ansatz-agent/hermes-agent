"""CLI handlers for the ``ansatz proxy`` subcommand."""
...
def _print_aiohttp_missing() -> None:
    print(
        "ansatz proxy requires aiohttp. Run `ansatz setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"ansatz provider add {adapter.name}")
        print(
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print(
        "\nStart the proxy with: ansatz proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``ansatz proxy <subcommand>``."""
    ...
    print(
        "ansatz proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  ansatz proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
