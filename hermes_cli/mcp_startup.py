...
def _resolve_discovery_timeout(
    ...
    When ``single_query`` is True (``ansatz -z "..."`` / ``-q``), the larger
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``ansatz dashboard``).  Those processes populate THIS module's
...
def ensure_mcp_discovery_before_agent_build(
    *,
    ...
    Non-interactive first turns (``chat -q``, ``ansatz -z``) can construct
