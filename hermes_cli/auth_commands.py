...
def auth_add_command(args) -> None:
    ...
    if provider == "nous":
        ...
        # full device-code flow. This makes `ansatz --profile <name>
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``ansatz provider add openai-codex`` overwrote
    ...
    if provider == "xai-oauth":
        ...
        # The singleton round-trip collapsed every added account into the
        # latest login: a second ``ansatz provider add xai-oauth`` overwrote the
    ...
    raise SystemExit(f"`ansatz provider add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `ansatz auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `ansatz auth` is called bare."""
