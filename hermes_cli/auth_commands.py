...
def auth_add_command(args) -> None:
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``hermes provider add openai-codex`` overwrote
    ...
    if provider == "xai-oauth":
        ...
        # The singleton round-trip collapsed every added account into the
        # latest login: a second ``hermes provider add xai-oauth`` overwrote the
    ...
    raise SystemExit(f"`hermes provider add {provider}` is not implemented for auth type {requested_type} yet.")
