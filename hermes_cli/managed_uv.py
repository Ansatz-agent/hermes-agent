...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            ...
            "out of WAL mode on this SQLite build. The next `ansatz update` "
            "will retry."
        )
        return
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``ansatz update`` runs
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``ansatz update`` daily;
...
def update_managed_uv(
    *,
    ...
    Call this during ``ansatz update`` so the managed copy stays current.
...
def _reload_hermes_constants():
    ...
    ``ansatz update`` imports ``hermes_constants`` from the OLD checkout,
...
def _install_safe_python_generation(
    ...
    *,
    ...
    # user isn't stuck on every `ansatz update` with no path to a fixed
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``ansatz update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``ansatz update`` repairs the runtime (issue class:
