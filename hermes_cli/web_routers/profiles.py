...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``ansatz profile use`` —
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``ansatz profile use``).
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``ansatz profile describe
    <name> --auto``.
...
# import_profile (the same machinery behind `ansatz profile export|import`).
