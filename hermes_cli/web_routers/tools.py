...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # same background install `ansatz tools` runs interactively. Without this,
    ...
    # dead-end that forced users to discover `ansatz computer-use install`
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``ansatz tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `ansatz tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``ansatz tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    the same store ``ansatz tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``ansatz tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``ansatz -p <profile> tools post-setup``.
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``ansatz computer-use permissions grant`` as a background action.
