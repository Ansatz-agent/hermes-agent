"""``ansatz debug`` debug tools for Hermes Agent.

Currently supports:
    ansatz debug share    Upload debug report (system info + logs) to a
...
_REDACTION_BANNER = (
    "[ansatz debug share: log content redacted at upload time. "
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``ansatz debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``ansatz debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `ansatz debug share` "
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``ansatz debug share`` invocation added ~20 MB of resident Python
    ...
    ``ansatz debug share`` also runs an opportunistic sweep as a fallback
...
# desktop app talks to a remote/docker/SSH backend, `ansatz debug share` runs
...
def _capture_dump() -> str:
    """Run ``ansatz dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``ansatz dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``ansatz debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "ansatz debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `ansatz debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  ansatz debug delete <url>")
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``ansatz debug share --nous``: upload the bundle to Nous-S3.
    ...
    if redact:
        logger.info(
            "ansatz debug share --nous: applied force-mode redaction before upload"
        )
    ...
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `ansatz debug share --local` to print the report instead, "
            "or `ansatz debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: ansatz debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'ansatz debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``ansatz debug`` call.
    ...
    # best-effort — any failure is swallowed so ``ansatz debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: ansatz debug <command>")
        print()
        print("Commands:")
