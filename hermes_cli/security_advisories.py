...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``ansatz doctor --ack <id>``; the ack is persisted to
...
1. ``ansatz doctor`` (and ``ansatz doctor --ack <id>``)
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        remediation=(
            ...
            "After cleanup: ansatz doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'ansatz doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``ansatz doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `ansatz doctor` on the gateway host for details.")
