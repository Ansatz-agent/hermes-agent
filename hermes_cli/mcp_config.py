"""
MCP Server Management CLI — ``ansatz mcp`` subcommand.

Implements ``ansatz mcp add/remove/list/test/configure`` for interactive
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``ansatz mcp login`` to distinguish a genuine authentication
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  ansatz mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  ansatz mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  ansatz mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: ansatz mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # earlier `ansatz mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  ansatz mcp add <name> --url <endpoint>')
        _info('  ansatz mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``ansatz mcp login`` and
    ``ansatz mcp reauth`` so both behave identically for a single server.
    """
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `ansatz mcp remove` + `ansatz mcp add` to reconfigure auth.")
        return False
    ...
    #
    # force_interactive_oauth: `ansatz mcp login` is *explicitly* user-
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `ansatz mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    ``ansatz mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``ansatz mcp reauth --all`` discovers every ``auth: oauth`` server in
    ...
    if not name:
        ...
        _info("Usage: ansatz mcp reauth <name>   |   ansatz mcp reauth --all")
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
# ─── ansatz mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'ansatz mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``ansatz mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("ansatz mcp                                    Open the catalog picker (default)")
        _info("ansatz mcp catalog                            List Nous-approved MCPs")
        _info("ansatz mcp install <name>                     Install a catalog MCP")
        _info("ansatz mcp serve                              Run as MCP server")
        _info("ansatz mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("ansatz mcp add <name> --command <cmd>         Add a stdio server")
        _info("ansatz mcp add <name> --preset <preset>       Add from a known preset")
        _info("ansatz mcp remove <name>                      Remove a server")
        _info("ansatz mcp list                               List configured servers")
        _info("ansatz mcp test <name>                        Test connection")
        _info("ansatz mcp configure <name>                   Toggle tools")
        _info("ansatz mcp login <name>                       Re-authenticate OAuth")
        _info("ansatz mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
