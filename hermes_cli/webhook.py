"""ansatz webhook — manage dynamic webhook subscriptions from the CLI.
...
def _setup_hint() -> str:
  ...
  Then start the gateway: ansatz gateway run
"""
...
def webhook_command(args):
    """Entry point for 'ansatz webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: ansatz webhook {subscribe|list|remove|test}")
        print("Run 'ansatz webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (ansatz gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: ansatz webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from ansatz webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (ansatz gateway run)")
