"""`ansatz checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.hermes/checkpoints/``.  Actions:

    ansatz checkpoints               # same as `status`
    ansatz checkpoints status        # total size, project count, breakdown
    ansatz checkpoints list          # per-project checkpoint counts + workdir
    ansatz checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    ansatz checkpoints clear [-f]    # nuke the entire base (asks first)
    ansatz checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    ansatz checkpoints
    ansatz checkpoints prune --retention-days 3 --max-size-mb 200
    ansatz checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: ansatz checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``ansatz checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `ansatz checkpoints` → status
