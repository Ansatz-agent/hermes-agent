...
Usage:
    ...
    ansatz chat                # Interactive chat
    ansatz gateway             # Run gateway in foreground
    ansatz gateway start       # Start gateway as service
    ansatz gateway stop        # Stop gateway service
    ansatz gateway status      # Show gateway status
    ansatz gateway install     # Install gateway service
    ansatz gateway uninstall   # Uninstall gateway service
    ansatz setup               # Interactive setup wizard
    ansatz logout              # Clear stored authentication
    ansatz status              # Show status of all components
    ansatz cron                # Manage cron jobs
    ansatz cron list           # List cron jobs
    ansatz cron status         # Check if cron scheduler is running
    ansatz doctor              # Check configuration and dependencies
    ansatz honcho setup                    # Configure Honcho AI memory integration
    ansatz honcho status                   # Show Honcho config and connection status
    ansatz honcho sessions                 # List directory → session name mappings
    ansatz honcho map <name>               # Map current directory to a session name
    ansatz honcho peer                     # Show peer names and dialectic settings
    ansatz honcho peer --user NAME         # Set user peer name
    ansatz honcho peer --ai NAME           # Set AI peer name
    ansatz honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    ansatz honcho mode                     # Show current memory mode
    ansatz honcho mode [hybrid|honcho|local]  # Set memory mode
    ansatz honcho tokens                   # Show token budget settings
    ansatz honcho tokens --context N       # Set session.context() token cap
    ansatz honcho tokens --dialectic N     # Set dialectic result char cap
    ansatz honcho identity                 # Show AI peer identity representation
    ansatz honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    ansatz honcho migrate                  # Step-by-step migration guide: OpenClaw native → Hermes + Honcho
    ansatz version             Show version
    ansatz update              Update to latest version
    ansatz uninstall           Uninstall Hermes Agent
    ansatz acp                 Run as an ACP server for editor integration
    ansatz sessions browse     Interactive session picker with search

    ansatz claw migrate --dry-run  # Preview migration without changes
"""
...
# When the user upgrades code via ``git pull`` (or ``ansatz update``
...
# ``ansatz update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _set_process_title() -> None:
    ...
    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``ansatz tools`` or
...
def _suppress_mouse_residue_early() -> None:
    ...
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`ansatz --tui … >log`, CI capture):
...
def _try_ultrafast_version() -> bool:
    """Handle ``ansatz --version`` before config/logging imports."""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (ansatz tools, ansatz setup, ansatz model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: '{CANONICAL_COMMAND} {command_name}' requires an interactive terminal.\n"
...
def _apply_profile_override() -> None:
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `ansatz mcp add ... --args <command argv>`.
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo ansatz -p <name>` against the invoking user's home.
    ...
    # after the subcommand (`ansatz chat -p coder`), so keep scanning broadly.
    ...
    # `ansatz profile use` and the gateway should honour that choice.
    # See issue #22502.
    ...
    # reserved ``gateway-default`` slot runs bare ``ansatz gateway run`` to mean
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``ansatz -c`` from repo A continues repo A's last session rather than the
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo {CANONICAL_COMMAND} {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo {CANONICAL_COMMAND} {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  ansatz --tui --resume {target}")
    if title:
        print(f'  ansatz --tui -c "{title}"')
    print()
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``ansatz update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    print(
        ...
        "This usually means `ansatz update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Hermes checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `ansatz --tui`\n"
        "If the checkout is still inconsistent, run `ansatz update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _launch_tui(
    ...
    # in the user's shell would otherwise make a plain `ansatz --tui` try to
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `ansatz update` so
...
def _pin_kanban_board_env() -> None:
    ...
    (``ansatz kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``ansatz kanban boards switch`` from another session can flip the file
...
def cmd_chat(args):
    ...
    if isinstance(_resume_raw, str) and _resume_raw.strip().lower() == "latest":
        ...
        if _last_id:
            ...
            print("Use 'ansatz sessions list' to see available sessions.")
            sys.exit(1)
    ...
    try:
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'ansatz doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'ansatz setup' at any time to configure.")
        sys.exit(1)
    ...
    # rare launch right after `ansatz update` changed a bundled skill.
...
def cmd_whatsapp(args):
    ...
    # has no creds.json.  Every subsequent `ansatz gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: ansatz gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `ansatz gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  ansatz gateway")
        ...
        print()
        print("  Or install as a service: ansatz gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'ansatz whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``ansatz whatsapp`` (the Baileys bridge wizard) — the
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``ansatz model``) and the setup wizard
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'ansatz model' for "
                "available providers, or run 'ansatz doctor' to diagnose config "
                "issues."
            )
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``ansatz model``
...
# `ansatz model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `ansatz model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``ansatz model`` flow, then route aux tasks
    to them here.
    """
    from hermes_cli.config import load_config
...
# fast paths like `ansatz --version` and slash-command dispatch that never
...
# every `hermes` invocation, including fast paths like `ansatz --version`
...
def _prompt_api_key(
    ...
    """Shared API-key entry point for ``ansatz setup`` / ``ansatz model``.
    ...
    if choice.startswith("c") and not pool_backed:
        ...
        print(
            f"  API key cleared.  Re-run `ansatz setup` to configure {pconfig.name} again."
        )
        return "", True
...
def cmd_sync(args):
        ...
        if status.get("org_available"):
            ...
            if modified:
                print(
                    ...
                    f"{', '.join(modified)}\n"
                    f"  Share them back with `ansatz sync propose <skill>`. "
    ...
    try:
        if sub == "pull":
            ...
            result = ssc.push_skills(identity=identity, message="ansatz sync push")
        elif sub == "now":
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message="ansatz sync now")
...
def cmd_slack(args):
    ...
    Dispatches ``ansatz slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: ansatz slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `ansatz slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1
...
def cmd_security(args):
    """Dispatch `ansatz security <subcmd>`."""
...
def cmd_approvals(args):
    """Dispatch `ansatz approvals <subcmd>`."""
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``ansatz update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``ansatz update``
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``ansatz update``
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``ansatz update`` to stash the
    lockfile — repeatedly.
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # as `ansatz update`'s _update_node_dependencies() (ui-tui + web +
...
#   - ``ansatz update`` can unconditionally call ``ansatz desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``ansatz desktop`` (interactive launch) skips the build when the
...
def _renderer_bundle_torn(dist_dir: Path) -> bool:
    ...
    the intact SOURCE tree, ``ansatz desktop`` skips the rebuild that would fix
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``ansatz update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        # Ansatz is the shipped product name. Keep Hermes as a compatibility
        # candidate for installs produced before the rebrand.
        candidates = [
            *release_dir.glob("mac*/Ansatz.app/Contents/MacOS/Ansatz"),
            *release_dir.glob("mac*/Hermes.app/Contents/MacOS/Hermes"),
        ]
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Ansatz.exe",
            release_dir / "win-unpacked" / "Hermes.exe",
            release_dir / "win-ia32-unpacked" / "Ansatz.exe",
            release_dir / "win-ia32-unpacked" / "Hermes.exe",
            release_dir / "win-arm64-unpacked" / "Ansatz.exe",
            release_dir / "win-arm64-unpacked" / "Hermes.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "Ansatz",
            release_dir / "linux-unpacked" / "hermes",
            release_dir / "linux-unpacked" / "Hermes",
            release_dir / "linux-arm64-unpacked" / "Ansatz",
...
# `ansatz update` → `ansatz desktop --build-only` → relaunch) rebuilds
...
def _ensure_desktop_exe_launchable(
    ...
    if restored is not None:
        ...
        print("    Your existing version was kept and still works. Run `ansatz desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `ansatz desktop --force-build` to rebuild, or re-run the Hermes")
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``ansatz desktop --build-only`` → electron-builder ``--dir``).
...
def _detect_linux_password_store() -> str | None:
    ...
    fails under `ansatz desktop` because the launcher environment doesn't look
...
def _register_linux_desktop_entry() -> None:
    ...
    shell. ``ansatz uninstall --gui`` removes it.
    """
    try:
...
def cmd_gui(args: argparse.Namespace):
    ...
    # `HERMES_DESKTOP_DISABLE_GPU=... ansatz desktop` keeps working.
    ...
    # `HERMES_DESKTOP_PASSWORD_STORE=... ansatz desktop` keeps working.
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  ansatz gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # ansatz update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `ansatz update` path).
            ...
            if build_result.returncode != 0:
                ...
                print("    ELECTRON_MIRROR=<mirror-base-url> ansatz desktop --force-build")
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``ansatz update``.
...
# =========================================================================
# Fork detection and upstream management for `ansatz update`
# =========================================================================
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``ansatz update``.
    ...
    protocol stream (``ansatz acp`` speaks JSON-RPC on stdout) must never get
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``ansatz update`` is still progressing even if pip/uv itself is silent.
    """
...
def _quarantine_running_hermes_exe(
    ...
    when ``ansatz update`` runs, ``hermes.exe`` IS the live process, and uv
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            ...
            "gateway, or pause AV scanning, then re-run `ansatz update`."
        )
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `ansatz update`:")
    print(
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    # hours later inside ``ansatz update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    ``hermes.exe`` shim is locked during ``ansatz update``, or when uv/distlib
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `ansatz update --force` after "
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``ansatz update``, not the venv we just wrote
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `ansatz update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `ansatz update --force` after closing other hermes processes."
        )
        return
...
class _UpdateOutputStream:
    """Stream wrapper used during ``ansatz update`` to survive terminal loss.
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``ansatz update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``ansatz update`` in an SSH session or a terminal
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``ansatz update --gateway``) the update is already
...
def cmd_update(args):
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `ansatz update --branch=<x>` would actually pull.
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``ansatz -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `ansatz profile` — show current profile status
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → ansatz -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  ansatz profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     ansatz -p {name} chat")
                else:
        ...
        if dist_name:
            ...
            print(f"  (run `ansatz profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → ansatz -p {name}  ({wrapper})")
        print()
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  ansatz -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      ansatz -p {plan.manifest.name} chat")
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `ansatz profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"ansatz -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print("No ansatz dashboard processes running.")
        return 0

    print(f"{len(live)} ansatz dashboard process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``ansatz dashboard register`` for OAuth.
    ...
    print("    [2] OAuth via Nous Portal (run `ansatz dashboard register`)")
    print("    [3] Cancel")
    print()
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    ``.../app.asar.unpacked/dist``. A standalone ``ansatz dashboard`` that
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No ansatz dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `ansatz update`.
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with ansatz serve")
    ...
    # those vars then runs `ansatz dashboard` would otherwise:
...
# so that in ``ansatz -m gpt5 chat``, ``gpt5`` is correctly skipped as a
...
def _first_positional_argv() -> str | None:
    ...
    ``ansatz -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``ansatz <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``ansatz photon``
...
def _try_fast_chat_launch() -> bool:
    ...
    ``hermes`` / ``ansatz -w -s foo --yolo`` / ``ansatz chat`` don't need the
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `ansatz --tui` is the hot path on phones. The full parser setup imports
...
def _main():
    ...
    # ``ansatz update`` runs on Windows. Silent no-op on non-Windows or when
    ...
    # If the checkout changed since the last launch (ansatz update, manual
    ...
    # Self-heal a venv left half-built by an interrupted ``ansatz update``
    ...
    # ``ansatz skills install update``) merely defers recovery one launch;
    # under-matching (missing ``ansatz -p work update``) would race a recovery
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `ansatz model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    # `ansatz proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``ansatz proxy`` subparser (dest='proxy_command').
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `ansatz whatsapp` which sets up "
    ...
    # subcommand — ``ansatz --help``, ``ansatz version``, ``ansatz logs``,
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `ansatz computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `ansatz tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `ansatz computer-use doctor` to run cua-driver's\n"
            ...
            if path:
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: ansatz computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: ansatz computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: ansatz computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: ansatz computer-use install")
            return
        if action == "doctor":
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: ansatz computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: ansatz computer-use permissions grant")
    ...
    # run `ansatz desktop` from a terminal, so the canonical name needs
    ...
    # e.g. ``ansatz -c Pokemon Agent Dev`` → ``ansatz -c 'Pokemon Agent Dev'``
    ...
    # fails (e.g. 'ansatz -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (ansatz hooks list, cron
    ...
    # ``ansatz egress start`` refusing when credential_source=bitwarden
