...
``ansatz gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``ansatz gateway start/stop/restart``).
    """
    ...
    # `ansatz gateway start/stop/restart` fell through to host code that spawns
...
# active (set via the ``ansatz -p <profile>`` flag before the call). This
...
#
# Per-profile gateways are registered dynamically when `ansatz profile create`
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``ansatz -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`ansatz profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          ...
          4. Drops to the hermes user and exec's
             ``ansatz -p <profile> gateway run`` (or just ``ansatz
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``ansatz gateway
        ...
        # (a stray ``ansatz gateway run`` from a shell, an agent action, or
        ...
        if profile == "default":
            gateway_cmd = "ansatz gateway run --replace"
        else:
            gateway_cmd = f"ansatz -p {shlex.quote(profile)} gateway run --replace"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``ansatz logs`` reads and what
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``ansatz gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``ansatz -p <profile> gateway start``.
