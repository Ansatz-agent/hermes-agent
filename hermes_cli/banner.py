...
_UPSTREAM_REPO_URL = "https://github.com/Ansatz-agent/hermes-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/ansatz-agent/hermes-agent"
...
def _github_compare_behind(current_rev: str, target_rev: str) -> Optional[int]:
    ...
    if not (_is_full_sha(current_rev) and _is_full_sha(target_rev)):
        return None
    url = (
        "https://api.github.com/repos/ansatz-agent/hermes-agent/"
...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    if _is_official_ssh_remote(origin_url):
        ...
        # ahead checkout as behind nudges the user into `ansatz update`,
...
_RELEASE_URL_BASE = "https://github.com/Ansatz-agent/hermes-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    canonical Ansatz-agent/hermes-agent repo (forks don't get a link).
    """
    ...
    if (provider or "").strip().lower() == "moa":
        ...
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            left_lines.append(
                ...
                f"[dim {dim}]— run /model or ansatz setup[/]"
            )
        else:
