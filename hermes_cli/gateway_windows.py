...
def _build_gateway_cmd_script(
    ...
    to that terminal like a normal foreground ``ansatz gateway run``.
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `ansatz -p <profile> gateway install` must be safe.
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"ansatz {profile_arg} gateway start" if profile_arg else "ansatz gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: ansatz gateway start")
        return
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                ...
                if start_now:
                    ...
                    print("  Approve the Windows UAC prompt, then run: ansatz gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: ansatz gateway start")
        _print_next_steps()
        return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: ansatz gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: ansatz gateway start")
            return
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `ansatz gateway status`."
        )
