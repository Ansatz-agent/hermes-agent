...
``ansatz -p <profile>`` joins the same board as the dispatcher that
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``ansatz kanban boards
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``ansatz kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    2. ``<root>/kanban/current`` on disk (set by ``ansatz kanban boards
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``ansatz kanban boards switch <typo>`` returns an error
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``ansatz kanban log`` unambiguous even when multiple
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``ansatz gateway run --replace`` /
...
def repair_db(
    ...
    *,
    ...
    :class:`RepairResult` so ``ansatz kanban repair`` can report and choose
    its own exit code.
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
    ...
    # holder (e.g. an external `ansatz kanban list` mid-integrity-probe) block
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``ansatz kanban init`` and the
...
def create_task(
    ...
    *,
    ...
    each name to ``ansatz --skills ...``. Use this to pin a task to a
    ...
    # `ansatz --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``ansatz kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `ansatz kanban recompute` (issue #22459).
...
def store_attachment_bytes(
    ...
    *,
    ...
    (``ansatz kanban attach``) so name-sanitisation, the size cap, and the
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``ansatz kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``ansatz kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``ansatz kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``ansatz kanban complete <id>``) works without requiring
...
#      via ``ansatz kanban show <id>`` and the dashboard.
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``ansatz kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``ansatz kanban
...
def _dispatch_once_locked(
    ...
    *,
    ...
    for row in ready_rows:
        ...
        # `_default_spawn` invokes ``ansatz -p <assignee>`` which fails
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `ansatz kanban tail` — without
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``ansatz -p <profile> chat -q ...`` subprocess.
    ...
    # env, and when the child process starts `ansatz -p <name>` the
    ...
    # read on the board and in `ansatz kanban log` — it is not a conversation
    ...
    # that even when the worker activates a profile (`ansatz -p <name>`
    ...
    # `ansatz -p <assignee>` activates the profile, but the env var is
    ...
    # `ansatz kanban log` on a specific board reads its own file and
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``ansatz kanban daemon`` is systemd-friendly.
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``ansatz kanban assignees`` for the terminal.
