"""
ansatz fallback — manage the fallback provider chain.
...
Subcommands:
  ansatz fallback [list]   Show the current fallback chain (default when no subcommand)
  ansatz fallback add      Pick provider + model via the same picker as `ansatz model`,
                           then append the selection to the chain
  ansatz fallback remove   Pick an entry to delete from the chain
  ansatz fallback clear    Remove all fallback entries
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `ansatz model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  ansatz fallback add")
        print()
        return
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `ansatz model`, then append the selection to the chain."""
    ...
    print("  `ansatz model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `ansatz fallback list` to view, or `ansatz fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``ansatz fallback [subcommand]``."""
