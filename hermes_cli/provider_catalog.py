...
The provider list shown by ``ansatz model`` (CLI/TUI) and the desktop Settings
...
the same universe ``ansatz model`` renders (``CANONICAL_PROVIDERS``), joining:
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``ansatz model`` shows.
"""
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `ansatz model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``ansatz model`` universe.
