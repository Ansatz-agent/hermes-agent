"""``ansatz sync`` subcommand parser — Skill Sync.
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    ansatz sync status                 show gate/opt-in/head state
    ansatz sync pull                   pull and materialize opted-in skills
    ansatz sync push                   push opted-in skills
    ansatz sync now                    reconcile: pull then push
    ansatz sync enable <skill>         opt a skill into sync
    ansatz sync disable <skill>        opt a skill out of sync
    ansatz sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    ansatz sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  ansatz sync status            what is synced, and from where\n"
            "  ansatz sync enable my-skill   include a skill in your sync\n"
            "  ansatz sync now               pull, then push\n"
            "  ansatz sync propose my-skill  share a skill with your team\n"
        ),
