...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use 'ansatz mcp add' to connect to a new server, or\n"
            "'ansatz mcp serve' to expose Hermes conversations over MCP."
        ),
    )
    ...
    # flag is omitted, causing `ansatz mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `ansatz mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `ansatz mcp install n8n`)",
    )
