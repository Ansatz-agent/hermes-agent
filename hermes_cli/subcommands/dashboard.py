"""``ansatz dashboard`` / ``ansatz serve`` subcommand parsers.
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `ansatz dashboard` / `ansatz serve` cmdlines and SIGTERM them directly —
    # the same path `ansatz update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
...
def build_dashboard_parser(
    ...
    # backend as `ansatz dashboard --no-open --tui --host ... --port ...`. The
    ...
    # `ansatz dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.hermes/.env.
    # Nested subparser so bare `ansatz dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "how to engage the login gate. Requires being logged in (ansatz setup)."
        ),
    )
