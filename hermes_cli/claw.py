"""ansatz claw — OpenClaw migration commands.

Usage:
    ansatz claw migrate              # Preview then migrate (always shows preview first)
    ansatz claw migrate --dry-run    # Preview only, no changes
    ansatz claw migrate --yes        # Skip confirmation prompt
    ansatz claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    ansatz claw migrate --no-backup  # Skip pre-migration snapshot
    ansatz claw cleanup              # Archive leftover OpenClaw directories
    ansatz claw cleanup --dry-run    # Preview what would be archived
"""
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    ...
    print_info("Recommendation: stop the gateway first with 'ansatz gateway stop'.")
    print()
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: ansatz claw <command> [options]")
        print()
        print("Commands:")
        ...
        print()
        print("Run 'ansatz claw <command> --help' for options.")
...
def _cmd_migrate(args):
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: ansatz claw migrate --source /path/to/.openclaw")
        return
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: ansatz claw migrate --yes")
            return
    ...
    # restorable with `ansatz import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            ...
            if backup_archive:
                ...
                print_info(f"Restore with: ansatz import {backup_archive.name}")
        except Exception as e:
            print()
    ...
    # 'ansatz claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: ansatz claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: ansatz claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  ansatz claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  ansatz claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  ansatz config set OPENROUTER_API_KEY sk-or-v1-...")
