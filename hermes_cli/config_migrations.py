...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_hermes_home()}/config.yaml and run `ansatz setup` to "
        ...
        if not quiet:
            if grandfathered:
                ...
                print(
                    ...
                    "Use `ansatz plugins enable <name>` to activate."
                )
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # settings in their `config.yaml`, and `ansatz curator status` has no
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `ansatz config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `ansatz config set`"
                )
...
# behavior set it to true explicitly via `ansatz config set`.
