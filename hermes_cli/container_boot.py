...
def reconcile_profile_gateways(
    *,
    ...
    is what ``ansatz gateway start`` (no ``-p``) targets. Without it,
    bare ``ansatz gateway start`` inside the container would land on
    ...
    # ``ansatz gateway start`` (no ``-p``) has somewhere to land;
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `ansatz profile create` (config.yaml
            # is not — that comes later via `ansatz setup`). Use it as the
            ...
            # via ``ansatz -p default-named gateway start`` if they
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    ...
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
    ...
    ``init`` prefix. The wrapper re-execs ``ansatz <subcommand>``, so an
    ...
    # The wrapper re-execs `ansatz <subcommand>`; peel an explicit hermes.
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``ansatz dashboard ...``) never spawns or
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `ansatz -p <profile> gateway start`
