"""``ansatz portal`` — the human-readable entry point for Nous Portal.

Running ``ansatz portal`` with no subcommand performs the one-shot Portal
...
``ansatz provider add nous --type oauth`` (which still works), is identical to
``ansatz setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``ansatz auth`` or ``ansatz tools``. It's the onboarding + discovery
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `ansatz model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `ansatz portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `ansatz provider add nous --type
    oauth`. It reuses the exact wiring behind `ansatz setup --portal` (which in
...
def portal_command(args) -> int:
    """Top-level dispatch for `ansatz portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `ansatz portal` is the
        # human-readable alias for `ansatz provider add nous --type oauth` /
        # `ansatz setup --portal`.
    ...
    print("Run `ansatz portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `ansatz portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `ansatz portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `ansatz provider add "
            "nous --type oauth`, identical to `ansatz setup --portal`). "
