"""``ansatz slack ...`` CLI subcommands.

Today only ``ansatz slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ ansatz slack manifest > slack-manifest.json
    # or:
...
def slack_manifest_command(args) -> int:
    ...
    if getattr(args, "slashes_only", False) and (
        ...
        print(
            "ansatz slack manifest: long description options cannot be used "
            "with --slashes-only",
            file=sys.stderr,
        )
        return 2
    if long_description_file is not None:
        ...
        try:
            ...
            print(
                f"ansatz slack manifest: cannot read long description from "
                f"{source_arg}: {exc}",
                file=sys.stderr,
            )
            return 2
    if (
        ...
        print(
            "ansatz slack manifest: long description must be at least "
    ...
    if (
        ...
        print(
            "ansatz slack manifest: long description must be at most "
