"""``ansatz logs`` — view and filter Hermes log files.
...
Usage examples::
    ...
    ansatz logs                    # last 50 lines of agent.log
    ansatz logs -f                 # follow agent.log in real time
    ansatz logs errors             # last 50 lines of errors.log
    ansatz logs gateway -n 100    # last 100 lines of gateway.log
    ansatz logs gui -f            # follow gui.log (dashboard/pty/ws)
    ansatz logs desktop -f        # follow desktop.log (Electron app boot/backend)
    ansatz logs --level WARNING    # only WARNING+ lines
    ansatz logs --session abc123   # filter by session ID substring
    ansatz logs --component tools  # only tool-related lines
    ansatz logs --since 1h         # lines from the last hour
    ansatz logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print("(Logs are created when Hermes runs — try 'ansatz chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'ansatz chat' to generate logs)")
