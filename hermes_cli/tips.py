...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or ansatz -c.",
    ...
    # --- CLI Flags ---
    "ansatz -c resumes your most recent CLI session. ansatz -c \"project name\" resumes by title.",
    "ansatz -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "ansatz -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "ansatz chat -t web,terminal enables only specific toolsets for a focused session.",
    "ansatz chat -s github-pr-workflow preloads a skill at launch.",
    "ansatz chat -q \"query\" runs a single non-interactive query and exits.",
    "ansatz chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "ansatz chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "ansatz --yolo bypasses all dangerous command approval prompts for the entire session.",
    "ansatz chat --source telegram tags the session for filtering in ansatz sessions list.",
    "ansatz -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "ansatz doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "ansatz dump outputs a compact setup summary — great for bug reports.",
    "ansatz config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "ansatz config edit opens config.yaml in your default editor.",
    "ansatz config check scans for missing or stale configuration options.",
    "ansatz sessions browse opens an interactive session picker with search.",
    "ansatz sessions stats shows session counts by platform and database size.",
    "ansatz sessions prune --older-than 30 cleans up old sessions.",
    "ansatz skills search react --source skills-sh searches the skills.sh public directory.",
    "ansatz skills check scans installed hub skills for upstream updates.",
    "ansatz skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "ansatz skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "ansatz mcp add github --command npx adds MCP servers from the command line.",
    "ansatz mcp serve runs Hermes itself as an MCP server for other agents.",
    "ansatz provider add lets you add multiple API keys for credential pool rotation.",
    "ansatz completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "ansatz logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "ansatz backup creates a zip backup of your entire Hermes home directory.",
    "ansatz profile create coder creates an isolated profile that becomes its own command.",
    "ansatz profile create work --clone copies your current config and keys to a new profile.",
    "ansatz update syncs new bundled skills to ALL profiles automatically.",
    "ansatz gateway install sets up Hermes as a system service (systemd/launchd).",
    "ansatz memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "ansatz webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: ansatz tools disables unused tools, ansatz skills config trims skills down.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as ansatz -w).",
    ...
    "Profile names become shell commands — 'ansatz profile create coder' creates the 'coder' command.",
    "ansatz profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "ansatz sessions export backup.jsonl exports all sessions for backup or analysis.",
    "ansatz -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "ansatz skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: ansatz cron add --skill blogwatcher \"Check for new posts\".",
    ...
    "ansatz gateway install sets it up as a system service that starts on boot.",
    ...
    # --- MCP ---
    "ansatz mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "ansatz mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "ansatz mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — ansatz tools can toggle them per platform.",
    ...
    "ansatz chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "ansatz plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "ansatz acp runs Hermes as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "ansatz provider add nous or ansatz provider add openai-codex sets up OAuth-based providers.",
    ...
    "ansatz status --deep runs deeper diagnostic checks across all components.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'ansatz profile create ls' would be rejected.",
    "ansatz profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "ansatz claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via ansatz tools.",
    ...
    # --- Curator ---
    'ansatz curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "ansatz curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'ansatz curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'ansatz provider reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'ansatz dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'ansatz dashboard embeds the full Hermes TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'ansatz -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'ansatz chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'ansatz chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'ansatz chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "ansatz chat --source tool tags programmatic chats so they don't clutter ansatz sessions list.",
    'ansatz dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'ansatz sessions rename <ID> "new title" renames any past session; ansatz sessions delete <ID> removes one.',
    'ansatz import restores a full Hermes backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'ansatz fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'ansatz pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'ansatz setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'ansatz status --deep runs the full health sweep across every component; plain ansatz status is the quick view.',
    ...
    '--source tool chats are excluded from ansatz sessions list by default — set --source explicitly to see them.',
