"""MCP picker — interactive `ansatz mcp picker` (also the default `ansatz mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``ansatz mcp add``, lets them pick one, and routes to install / enable /
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `ansatz mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: ansatz mcp install <name>    Picker: ansatz mcp",
        Colors.DIM,
    ))
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Hermes — run `ansatz update` "
...
def show_catalog() -> None:
    """`ansatz mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`ansatz mcp picker` (and default `ansatz mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`ansatz mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `ansatz mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
