...
def _kanban_dispatch_allowed() -> bool:
    """Return False while the global emergency stop (`ansatz pause`) is engaged.
...
class GatewayKanbanWatchersMixin:
    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        ...
        no separate `ansatz kanban daemon` process needed. When false, the
            ...
            try:
                ...
                if _is_corrupt_board_db_error(exc):
                    ...
                    logger.error(
                        ...
                        "then run `ansatz kanban init` if you need a fresh board.",
                        slug,
                        fingerprint[0],
                    )
                    return None
        ...
        while self._running:
            ...
            try:
                # Global emergency stop (`ansatz pause`): skip auto-decompose
                ...
                if bad_ticks >= HEALTH_WINDOW:
                    ...
                    if now - last_warn_at >= 300:
                        logger.warning(
                            ...
                            "`ansatz kanban list --status ready`.",
                            bad_ticks,
                        )
                        last_warn_at = now
