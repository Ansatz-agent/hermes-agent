...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: ansatz gateway restart"
        ),
    )
...
class GatewaySlashCommandsMixin:
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`ansatz gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_heartbeat_command(self, event: "MessageEvent") -> str:
        ...
        return (
            ...
            "elapsed. Lives while the gateway runs — use `ansatz cron` for durable schedules."
        )
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        ``ansatz skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _hermes_home
        from hermes_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `ansatz skills diff <name>` is a
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `ansatz bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``ansatz debug share`` from the CLI.
        """
        import asyncio
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        Spawns ``ansatz update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``ansatz update`` may trigger. Marker
        ...
        # Spawn `ansatz update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `ansatz update --gateway`
