...
[Nous Portal](/user-guide/features/tool-gateway) 在单一订阅下提供 300+ 个模型。全新安装后，运行 `ansatz setup --portal` 即可登录并一键将 Nous 设为提供商。使用 `ansatz portal info` 查看当前配置。
:::
...
| **Skills Hub（技能中心）** | `ansatz skills search` 使用此槽位。通常保持 `auto` 即可。 |
...
- **CLI**（`ansatz chat`）：下次执行 `ansatz chat` 时生效。
- **Gateway**（Telegram、Discord、Slack 等）：下一个*新*会话生效。现有会话保持原有模型。如需强制所有会话使用新配置，重启 gateway（`ansatz gateway restart`）。
...
Hermes 仅列出具有有效凭据的提供商。检查侧边栏中的 **Keys** — 应存在以下之一：API key、成功的 OAuth 或自定义端点 URL。若所需提供商不在列表中，运行 `ansatz setup` 进行配置，或前往 **Keys** 添加环境变量。
...
```bash
ansatz model            # 交互式提供商 + 模型选择器（切换默认值的标准方式）
```

`ansatz model` 引导你选择提供商、完成认证（OAuth 流程会打开浏览器；API key 提供商会提示输入密钥），然后从该提供商的精选目录中选择具体模型。选择结果写入 `~/.hermes/config.yaml` 的 `model.provider` 和 `model.model` 字段。

如需在不启动选择器的情况下列出提供商/模型，请使用仪表板或下方的 REST 端点。查看 CLI 当前实际使用的配置：`ansatz config get model` 和 `ansatz status`。
