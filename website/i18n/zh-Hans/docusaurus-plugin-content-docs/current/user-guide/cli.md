...
:::tip
Hermes 还提供了一个现代 TUI，支持模态覆盖层、鼠标选择和非阻塞输入。使用 `ansatz --tui` 启动——参见 [TUI](tui.md) 指南。
:::
...
# 使用指定提供商
ansatz chat --provider nous        # 使用 Nous Portal
ansatz chat --provider openrouter  # 强制使用 OpenRouter
...
# 恢复之前的会话
ansatz --continue             # 恢复最近的 CLI 会话（-c）
ansatz --resume <session_id>  # 通过 ID 恢复指定会话（-r）

# 详细模式（调试输出）
ansatz chat --verbose

# 隔离的 git worktree（用于并行运行多个 agent）
ansatz -w                         # 在 worktree 中以交互模式运行
ansatz -w -z "Fix issue #123"     # 在 worktree 中以单次查询模式运行
```
...
| 时长 | 会话已用时间 |
| ⚠ YOLO | **YOLO 模式警告**——当 `HERMES_YOLO_MODE` 开启时显示（通过启动时的 `ansatz --yolo` 或会话中的 `/yolo` 切换）。与横幅行警告保持同步，确保你不会忘记自己处于自动批准模式。 |
...
恢复之前的会话时（`ansatz -c` 或 `ansatz --resume <id>`），横幅与输入提示符之间会出现一个"Previous Conversation"面板，显示对话历史的简洁摘要。详情及配置说明参见[会话——恢复时的对话摘要](sessions.md#conversation-recap-on-resume)。
...
```bash
ansatz --continue                          # 恢复最近的 CLI 会话
ansatz -c                                  # 简写形式
ansatz -c "my project"                     # 恢复命名会话（谱系中最新的）
ansatz --resume 20260225_143052_a1b2c3     # 通过 ID 恢复指定会话
ansatz --resume "refactoring auth"         # 通过标题恢复
ansatz -r 20260225_143052_a1b2c3           # 简写形式
```

恢复会从 SQLite 中还原完整的对话历史。agent 能看到所有之前的消息、工具调用和响应——就像从未离开一样。

在聊天中使用 `/title My Session Name` 为当前会话命名，或从命令行使用 `ansatz sessions rename <id> <title>`。使用 `ansatz sessions list` 浏览历史会话。
