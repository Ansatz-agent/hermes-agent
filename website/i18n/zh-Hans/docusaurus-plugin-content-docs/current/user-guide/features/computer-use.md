...
使用 `ansatz computer-use status` 验证安装结果。

**方式二：通过交互式界面启用工具集。**

1. 运行 `ansatz tools`，选择 `🖱️ Computer Use (macOS)` → `cua-driver (background)`。
...
- **`ansatz update`** — 更新 Hermes 本身时，如果 `cua-driver` 在 PATH 中，更新结束时会重新运行上游安装程序。对非 macOS 用户及未安装 cua-driver 的用户无操作。
- **`ansatz computer-use install --upgrade`** — 手动强制刷新。无论 cua-driver 是否已安装，都会重新运行上游安装程序。在不等待下次 Agent 更新的情况下获取最新修复时使用此命令。

`ansatz computer-use status` 会在二进制路径旁显示已安装的版本号。
...
**`computer_use backend unavailable: cua-driver is not installed`** — 运行 `ansatz computer-use install` 获取 cua-driver 二进制文件，或运行 `ansatz tools` 并启用 Computer Use 工具集。
