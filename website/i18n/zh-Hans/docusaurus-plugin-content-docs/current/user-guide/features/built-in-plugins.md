...
`plugins/memory/` 和 `plugins/context_engine/` 被刻意排除在内置扫描之外。这两个目录使用各自的发现路径，因为内存提供者和上下文引擎是通过 `ansatz memory setup` / 配置中的 `context.engine` 进行单选配置的提供者。

## 内置插件默认不启用

内置插件随附时处于禁用状态。发现机制会找到它们（它们会出现在 `ansatz plugins list` 和交互式 `ansatz plugins` UI 中），但在你明确启用之前不会加载：
...
仓库在 `plugins/` 下附带了以下内置插件。所有插件均需手动启用——通过 `ansatz plugins enable <name>` 启用。
...
内存提供者（`plugins/memory/*`）和上下文引擎（`plugins/context_engine/*`）在 [内存提供者](./memory-providers.md) 中单独列出——它们分别通过 `ansatz memory` 和 `ansatz plugins` 管理。以下是两个长期运行的基于 hook 的插件的详细说明。
...
**启用：** `ansatz plugins enable disk-cleanup`（或在 `ansatz plugins` 中勾选复选框）。

**再次禁用：** `ansatz plugins disable disk-cleanup`。

### observability/langfuse

将 Hermes 的轮次、LLM 调用和工具调用追踪到 [Langfuse](https://langfuse.com)——一个开源 LLM 可观测性平台。每轮一个 span，每次 API 调用一个 generation，每次工具调用一个 tool observation。用量总计、各类型 token 数量和成本估算来自 Hermes 的标准 `agent.usage_pricing` 数据，因此 Langfuse 仪表盘看到的分类（input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`）与 `ansatz logs` 中显示的一致。
...
或在交互式 `ansatz plugins` UI 中勾选复选框。然后将凭据写入 `~/.hermes/.env`：
...
会话分组基于 Hermes 会话 ID（或子 agent 的任务 ID），通过 `langfuse.propagate_attributes` 实现，因此单次 `ansatz chat` 会话中的所有内容都归属于同一个 Langfuse session。

**验证：**

```bash
ansatz plugins list                 # observability/langfuse 应显示 "enabled"
ansatz chat -q "hello"              # 在 Langfuse UI 中检查是否有 "Hermes turn" trace
```
...
**禁用：** `ansatz plugins disable observability/langfuse`。插件模块仍会被发现，但在你重新启用之前不会运行任何模块代码。
...
**禁用：** `ansatz plugins disable google_meet`。已缓存的转录和录音保留在 `~/.hermes/cache/google_meet/`，直到你手动删除。
...
**启用：** 无需启用——`hermes-achievements` 是一个仅限仪表盘的插件（无生命周期 hook，无模型可见工具）。它在 `ansatz dashboard` 首次启动时自动注册为标签页。`plugins.enabled` 配置仅控制生命周期/工具插件；仪表盘插件完全通过其 `dashboard/manifest.json` 发现。
...
- 在 `ansatz plugins list` 中，manifest 来源显示为 `bundled`
- 同名用户插件会覆盖内置版本
