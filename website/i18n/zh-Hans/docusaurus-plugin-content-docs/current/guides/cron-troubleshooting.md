...
如果你期望任务自动触发，需要运行一个 gateway（前台运行用 `ansatz gateway`，安装为服务用 `ansatz gateway start`）。如需单次调试，可手动触发一次 tick：`ansatz cron tick`。
...
```bash
date
ansatz cron list   # 将 next_run 时间与本地时间对比
```
...
若投递失败，任务仍会执行——只是不会发送到任何地方。检查 `ansatz cron list` 中的 `last_error` 字段（如有）。
...
Skill 必须先安装才能附加到 cron 任务。若 skill 缺失，先用 `ansatz skills install <skill-name>` 安装，或在 CLI 中通过 `/skills` 安装。

### 检查 2：检查 skill 名称与 skill 文件夹名称

Skill 名称区分大小写，必须与已安装 skill 的文件夹名称完全匹配。若任务指定的是 `ai-funding-daily-report`，但 skill 文件夹也是 `ai-funding-daily-report`，请从 `ansatz skills list` 确认确切名称。
...
3. 通过 `ansatz cron list` 查看任务的 `last_run` 元数据
...
Skill 必须安装在运行调度器的机器上。若你在不同机器间切换，skill 不会自动同步——请用 `ansatz skills install <skill-name>` 重新安装。
...
```bash
ansatz cron list                    # 显示所有任务、状态、next_run 时间
ansatz cron run <job_id>            # 安排在下次 tick 执行（用于测试）
ansatz cron edit <job_id>           # 修复配置问题
ansatz logs                         # 查看近期 Hermes 日志
ansatz skills list                  # 确认已安装的 skill
```
...
1. 使用 `ansatz cron run <job_id>` 运行任务（在下次 gateway tick 时触发），观察聊天输出中的错误
