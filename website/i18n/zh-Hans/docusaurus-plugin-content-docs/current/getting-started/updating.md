...
:::tip
`ansatz update` 会自动检测新的配置选项并提示你添加。如果跳过了该提示，可手动运行 `ansatz config check` 查看缺失的选项，再运行 `ansatz config migrate` 以交互方式添加。
:::

### 更新过程

运行 `ansatz update` 时，将依次执行以下步骤：
...
### 仅预览：`ansatz update --check`

想在拉取前确认是否有更新？运行 `ansatz update --check` — 它会获取并与 `origin/main` 比较提交。不修改任何文件，不重启 gateway。适合在以"是否有更新"为条件的脚本和 cron 任务中使用。
...
在 Windows 上，如果 `ansatz update` 检测到另一个 `hermes.exe` 进程持有 venv 入口点可执行文件的句柄，它将拒绝运行 — 最常见的情况是 Hermes Desktop 应用启动的后端进程、另一个终端中打开的 `hermes` REPL，或正在运行的 gateway：
...
`ansatz update` 处理主要的更新流程，但快速验证可确认一切正常落地：

1. `git status --short` — 若工作树出现意外的脏状态，请在继续前检查
2. `ansatz doctor` — 检查配置、依赖项和服务健康状态
3. `ansatz --version` — 确认版本已按预期更新
4. 如果使用 gateway：`ansatz gateway status`
5. 如果 `doctor` 报告 npm audit 问题：在标记的目录中运行 `npm audit fix`

:::warning 更新后工作树出现脏状态
如果 `ansatz update` 后 `git status --short` 显示意外变更，请在继续前停下来检查。这通常意味着本地修改被重新应用到了更新后的代码之上，或依赖步骤刷新了锁文件。
:::

### 终端在更新中途断开连接

`ansatz update` 针对意外终端断开进行了保护：
...
你不再需要将 `ansatz update` 包裹在 `screen` 或 `tmux` 中来应对终端断开。
...
:::warning
如果新增了配置选项，回滚可能导致配置不兼容。回滚后运行 `ansatz config check`，如果遇到错误，请从 `config.yaml` 中删除无法识别的选项。
:::
