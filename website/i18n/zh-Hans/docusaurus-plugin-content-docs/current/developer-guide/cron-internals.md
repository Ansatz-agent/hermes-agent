...
| `hermes_cli/cron.py` | CLI `ansatz cron` 子命令 |
...
在 CLI 模式下，cron 任务仅在运行 `ansatz cron` 命令或活跃 CLI 会话期间触发。
...
调度器使用跨进程文件锁（Unix 上的 `fcntl.flock`，Windows 上的 `msvcrt.locking`）防止重叠的 tick 对同一批到期任务执行两次——即使在 gateway 的进程内 ticker 与独立的 `ansatz cron` / 手动 `tick()` 调用之间也如此。若无法获取锁，`tick()` 立即返回 0。

## CLI 接口

`ansatz cron` CLI 提供直接的任务管理功能：

```bash
ansatz cron list                    # 显示所有任务
ansatz cron create                  # 交互式创建任务（别名：add）
ansatz cron edit <job_id>           # 编辑任务配置
ansatz cron pause <job_id>          # 暂停运行中的任务
ansatz cron resume <job_id>         # 恢复已暂停的任务
ansatz cron run <job_id>            # 触发立即执行
