...
:::tip
`hermes_cli/setup.py` **无需**修改。setup 向导将 provider/model 选择委托给 `main.py` 中的 `select_provider_and_model()`——在那里添加的任何 provider 都会自动出现在 `ansatz setup` 中。
:::
...
7. `ansatz model` 菜单包含该 provider
8. `ansatz setup` 向导自动委托给 `main.py`
...
- 带有特定认证流程的 provider 专属 `ansatz model` 菜单条目
...
在交互式 `ansatz model` 流程中出现之前，provider 是不可发现的。
...
:::tip
`hermes_cli/setup.py` 无需修改——它调用 `main.py` 中的 `select_provider_and_model()`，因此你的新 provider 会自动出现在 `ansatz model` 和 `ansatz setup` 中。
:::
...
### 7. 更新了 `ansatz model` 但未更新 `ansatz setup`
