...
- **Anthropic** — Claude 模型（直接 API、通过 `ansatz auth add anthropic` 进行 OAuth、OpenRouter 或任何兼容代理）
...
使用 `ansatz model` 设置提供商，或直接编辑 `~/.hermes/.env`。所有提供商 key 请参阅[环境变量](./environment-variables.md)参考文档。
...
可以。运行 `ansatz model`，选择**自定义端点**，然后输入您服务器的 URL：
...
**解决方案：** 退出当前会话，在终端中使用 `ansatz model` 添加新提供商：
...
通过 `ansatz model` 添加新提供商后，启动新的聊天会话 — `/model` 将显示所有已配置的提供商。

:::tip 快速参考
| 目标 | 使用方式 |
|-----------|-----|
| 添加新提供商 | `ansatz model`（从终端） |
| 输入/更改 API key | `ansatz model`（从终端） |
...
- 升级您的提供商套餐
- 切换到其他模型或提供商
- 使用 `ansatz chat --provider <alternative>` 路由到其他后端
...
**解决方案：**
- 使用 `ansatz gateway setup` 验证您的 bot token 是否有效
...
#### WSL：网关持续断开连接或 `ansatz gateway start` 失败
...
1. 创建一个任务，运行 `wsl -d Ubuntu -- bash -lc 'ansatz gateway run'`
2. 设置在用户登录时触发
:::
...
**解决方案：** 网关在您运行 `ansatz gateway install` 时会捕获您的 shell PATH。如果您在设置网关后安装了新工具，请重新运行 install 以捕获更新后的 PATH：

```bash
ansatz gateway install    # 重新快照当前 PATH
ansatz gateway start      # 检测到更新的 plist 并重新加载
```
...
**解决方案：**
- 尝试更快/更小的模型：`ansatz chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- 减少激活的工具集：`ansatz chat -t "terminal"`
- 检查到提供商的网络延迟
...
不共享。每个 profile 都有自己独立的记忆存储、会话数据库和技能目录，完全隔离。如果您想用现有的记忆和会话创建新 profile，请使用 `ansatz profile create newname --clone-all` 从当前 profile 复制所有内容，或添加 `--clone-from <profile>` 从指定源 profile 复制。

### 运行 `ansatz update` 时会发生什么？

`ansatz update` 拉取最新代码并重新安装依赖项**一次**（不是每个 profile 各一次）。然后自动将更新的技能同步到所有 profiles。您只需运行一次 `ansatz update` — 它覆盖机器上的每个 profile。
...
**场景：** Telegram 有 100 个 slash 命令的限制，您的技能数量已超过此限制。您想禁用 Telegram 上不需要的技能，但 `ansatz skills config` 设置似乎没有生效。

**解决方案：** 使用 `ansatz skills config` 按平台禁用技能。这会写入 `config.yaml`：
...
更改后，**重启网关**（`ansatz gateway restart` 或终止并重新启动）。Telegram bot 命令菜单在启动时重建。
...
4. 在新机器上运行 `ansatz setup` 以验证 API key 和提供商配置是否正常工作。
...
### `ansatz backup` 与 `ansatz profile export` 的对比

| 功能 | `ansatz backup` | `ansatz profile export` |
| :--- | :--- | :--- |
...
:::tip
`ansatz backup` 即使在 Hermes 正在运行时也能生成一致的快照。还原的归档文件不包含机器本地的运行时文件，如 `gateway.pid` 和 `cron.pid`。
:::
...
3. **提交 bug 报告：** 请包含您的操作系统、Python 版本（`python3 --version`）、Hermes 版本（`ansatz --version`）以及完整的错误信息