...
Every credential injected by a source is labelled with its origin — setup flows and `ansatz model` show `(from Bitwarden)` next to detected keys so you always know where a value came from.
