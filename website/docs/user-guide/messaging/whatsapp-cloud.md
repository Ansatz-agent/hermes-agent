...
3. Start the Hermes gateway: `ansatz gateway`.
...
The `ansatz whatsapp-cloud` wizard prints these links at the end of setup. None of this is required for the bot to work — it's pure polish for how your bot appears to users.
...
- **Gateway not running** — check `ansatz gateway` is up.
...
Common cause: the toolset configured for `whatsapp_cloud` is missing the tools the agent wants to call.  Check `ansatz tools list` and verify the platform is using `hermes-whatsapp` (the default Cloud adapter toolset, same as Baileys).
...
```bash
ansatz config set stt.provider openai
ansatz config set stt.use_gateway true
...
| | Baileys (`ansatz whatsapp`) | Cloud API (`ansatz whatsapp-cloud`) |
|---|---|---|
