...
> Run `ansatz gateway setup` and pick **IRC** for a guided walk-through.
...
Check status with `ansatz gateway status` — IRC connection state is reported there, including for env-only setups.
