...
`ansatz gateway enroll` refuses to run in managed/hosted installs — there the
...
token. Re-login with `ansatz auth add nous` (or `ansatz setup`) and retry.
...
startup. Restart it (`ansatz gateway restart`).
