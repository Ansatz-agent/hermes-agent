...
> Run `ansatz gateway setup` and pick **ntfy** for a guided walk-through.
...
Or target a specific topic explicitly via the cron job's `deliver:` field, or from a shell script with the [`ansatz send` CLI](/guides/pipe-script-output):

```bash
ansatz send ntfy:alerts-channel "Done!"
```
