...
> Run `ansatz gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
(Use `ansatz gateway start` only after `ansatz gateway install` has registered the systemd/launchd service.)
...
fixing, restart `ansatz gateway run`.
...
Check `ansatz gateway run` logs for the bound host/port. If the adapter bound to
