...
> Run `ansatz gateway setup` and pick **WhatsApp** for a guided walk-through.
...
3. Run `ansatz whatsapp` and scan the QR code from that WhatsApp account
...
| **QR code expires** | QR codes refresh every ~20 seconds. If it times out, restart `ansatz whatsapp`. |
...
| **Logged out unexpectedly** | WhatsApp unlinks devices after long inactivity. Keep the phone on and connected to the network, then re-pair with `ansatz whatsapp` if needed. |
...
| **macOS: "Node.js not installed" but node works in terminal** | launchd services don't inherit your shell PATH. Run `ansatz gateway install` to re-snapshot your current PATH into the plist, then `ansatz gateway start`. See the [Gateway Service docs](./index.md#macos-launchd) for details. |
