...
> Run `ansatz gateway setup` and pick **Google Chat** for a guided walk-through.
...
ansatz -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```
...
3. Check `ansatz gateway` logs for `[GoogleChat] Connected`. If you see
