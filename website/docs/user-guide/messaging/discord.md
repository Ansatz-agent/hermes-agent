...
:::tip
You can run `ansatz gateway` in the background or as a systemd service for persistent operation. See the deployment docs for details.
:::
...
No extra configuration is needed — any skill installed via `ansatz skills install` is automatically registered as a Discord slash command on the next gateway restart.
...
3. Click **Save Changes**, then restart the gateway (`ansatz gateway restart`).
...
**Fix**: Check that `ansatz gateway` is running. Verify `DISCORD_BOT_TOKEN` in your `.env` file. If you recently reset the token, update it.
