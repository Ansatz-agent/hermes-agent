...
> Run `ansatz gateway setup` and pick **Buzz** for a guided walk-through.
...
Check status with `ansatz gateway status` — Buzz connection state is reported there, including for env-only setups.
