...
- **QR code login** — scan-to-connect setup via `ansatz gateway setup`
...
1. Pair Hermes once with `ansatz gateway setup` and note the connected iLink bot
   account.
...
| `Weixin startup failed: WEIXIN_TOKEN is required` | Run `ansatz gateway setup` to complete QR login, or set `WEIXIN_TOKEN` manually |
| `Weixin startup failed: WEIXIN_ACCOUNT_ID is required` | Set `WEIXIN_ACCOUNT_ID` in your `.env` or run `ansatz gateway setup` |
...
| Session expired (`errcode=-14`) | Your login session has expired. Re-run `ansatz gateway setup` to scan a new QR code |
