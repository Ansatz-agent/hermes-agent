...
> Run `ansatz gateway setup` and pick **SimpleX** for a guided walk-through.
...
2. Use **DM pairing** — send any message to the bot and it will reply with a pairing code. Enter that code via `ansatz pairing approve simplex <CODE>`.
...
`simplex:group:12` as a cron `deliver=` target or in a `ansatz send` call.

## Sending with `ansatz send`

SimpleX works as a standalone send target — the daemon must be running,
but a live gateway is not required for plain text:

```bash
ansatz send --to simplex:alice "hello"          # DM by contact display name
ansatz send --to simplex:group:12 "hello"       # group by numeric ID
ansatz send --to simplex "hello"                # SIMPLEX_HOME_CHANNEL
```
...
`ansatz send --list` shows them by name. Before the first gateway run the
...
Or target a specific contact via the cron job's `deliver:` field, or from a shell script with the [`ansatz send` CLI](/guides/pipe-script-output):

```bash
ansatz send simplex:<contact-id> "Done!"
```
