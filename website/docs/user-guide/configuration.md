...
Run `ansatz setup --portal` — one OAuth gets you a model provider and all four Tool Gateway tools without hand-editing YAML. Portal subscribers also get 10% off token-billed providers. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
ansatz config              # View current configuration
ansatz config edit         # Open config.yaml in your editor
ansatz config get KEY      # Print a resolved value
ansatz config set KEY VAL  # Set a specific value
ansatz config unset KEY    # Remove a user-set value
ansatz config check        # Check for missing options (after updates)
ansatz config migrate      # Interactively add missing options

# Examples:
ansatz config get model
ansatz config set model anthropic/claude-opus-4
ansatz config set terminal.backend docker
ansatz config unset terminal.backend
ansatz config set OPENROUTER_API_KEY sk-or-...  # Saves to .env
```

:::tip
The `ansatz config set` command automatically routes values to the right file — API keys are saved to `.env`, everything else to `config.yaml`.
:::
...
1. **CLI arguments** — e.g., `ansatz chat --model anthropic/claude-sonnet-4` (per-invocation override)
...
`ansatz serve --isolated`) apply the configured `RLIMIT_NOFILE` soft limit
...
`ansatz update` settings live under `updates` in `config.yaml`:
...
:::warning
The agent has the same filesystem access as your user account. Use `ansatz tools` to disable tools you don't want, or switch to Docker for sandboxing.
:::
...
```bash
VERCEL_OIDC_TOKEN="$(vc project token <project-name>)" ansatz chat
```

From a linked Vercel project directory, you can omit the project name:

```bash
VERCEL_OIDC_TOKEN="$(vc project token)" ansatz chat
```
...
- **Docker** — Run `docker version` to verify Docker is working. If it fails, fix Docker or `ansatz config set terminal.backend local`.
- **SSH** — Both `TERMINAL_SSH_HOST` and `TERMINAL_SSH_USER` must be set. Hermes logs a clear error if either is missing.
- **Modal** — Needs `MODAL_TOKEN_ID` env var or `~/.modal.toml`. Run `ansatz doctor` to check.
...
Hermes resolves each listed variable from your current shell first, then falls back to `~/.hermes/.env` if it was saved with `ansatz config set`.
...
```bash
ansatz config set terminal.persistent_shell false
```
...
- `ansatz config migrate` scans all enabled skills, finds unconfigured settings, and offers to prompt you
- `ansatz config show` displays all skill settings under "Skill Settings" with the skill they belong to
- When a skill loads, its resolved config values are injected into the skill context automatically

**Setting values manually:**

```bash
ansatz config set skills.config.myplugin.path ~/myplugin-data
```
...
`ansatz tools`), so a toolset listed here is always removed — even if a
...
```yaml
worktree: true    # Always create a worktree (same as ansatz -w)
...
- **Needs-attention escalation.** A platform continuously in the retry queue past `agent.reconnect_attention_after` (default `7200` seconds = 2 hours, `0` disables) gets `needs_attention: true` and a `retrying_since` timestamp in gateway runtime status (`ansatz status`), plus a WARNING log. Retries continue unchanged — this is a signal, not a circuit breaker. The flag clears on successful reconnect.
...
Instead of hand-editing YAML, run `ansatz model` and pick **"Configure auxiliary models"** from the menu. You'll get an interactive per-task picker:
...
`/title` and `ansatz sessions rename`.
...
auxiliary:
  ...
  # Kanban triage specifier — `ansatz kanban specify <id>` (or the
...
| `"nous"` | Force Nous Portal | `ansatz auth` |
| `"codex"` | Force Codex OAuth (ChatGPT account). Supports vision (gpt-5.3-codex). | `ansatz model` → ChatGPT or Codex Subscription |
| `"minimax-oauth"` | Force MiniMax OAuth (browser login, no API key). Uses MiniMax-M2.7-highspeed for auxiliary tasks. | `ansatz model` → MiniMax (OAuth) |
| `"xai-oauth"` | Force xAI Grok OAuth (browser login for SuperGrok or X Premium+ subscribers, no API key). Same OAuth token covers chat, TTS, image, video, and transcription. | `ansatz model` → xAI Grok OAuth (SuperGrok / Premium+) |
| `"main"` | Use your active custom/main endpoint. This can come from `OPENAI_BASE_URL` + `OPENAI_API_KEY` or from a custom endpoint saved via `ansatz model` / `config.yaml`. Works with OpenAI, local models, or any OpenAI-compatible API. **Auxiliary tasks only — not valid for `model.provider`.** | Custom endpoint credentials + base URL |
...
```
Run `ansatz model` and select **MiniMax (OAuth)** to log in and set this automatically. For the China region, the base URL will be `https://api.minimaxi.com/anthropic`. See the [MiniMax OAuth guide](../guides/minimax-oauth.md) for the full walkthrough.
...
:::tip
Run `ansatz config` to see your current auxiliary model settings. Overrides only show up when they differ from the defaults.
:::
...
:::note
There is no `ansatz config set` support for `reasoning_overrides` keys — edit the YAML file directly. This is because model names often contain dots (e.g. `claude-opus-4.5`), which conflict with the CLI's dotted-key syntax.
:::
...
If writes to Hermes state (cron jobs, skills, scripts under `~/.hermes/`) are failing, check whether `HERMES_WRITE_SAFE_ROOT` is set in your environment. For cron changes, use the `cronjob` tool or `ansatz cron edit` instead of patching `jobs.json` directly.
...
Run `ansatz status` to see the current slot usage and every holder.
...
The `web_search` and `web_extract` tools support five backend providers. Configure the backend in `config.yaml` or via `ansatz tools`:
...
```yaml
checkpoints:
  enabled: false                 # Enable automatic checkpoints (also: ansatz chat --checkpoints). Default: false (opt-in).
