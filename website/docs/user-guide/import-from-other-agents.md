...
`ansatz import-agent` imports your existing **Claude Code** or **OpenAI Codex CLI** setup into Hermes with one command. It follows the same preview-first pattern as [`ansatz claw migrate`](../guides/migrate-from-openclaw.md): you always see a per-item plan before anything is written, and `--dry-run` never touches disk.
...
ansatz import-agent claude-code --dry-run          # preview only
ansatz import-agent codex --source /path/to/.codex # custom location
ansatz import-agent claude-code --overwrite --yes  # replace conflicts, skip prompts
```
...
- Coming from OpenClaw instead? Use [`ansatz claw migrate`](../guides/migrate-from-openclaw.md).
