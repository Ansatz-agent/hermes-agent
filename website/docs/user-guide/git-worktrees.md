...
- CLI: the directory where you run `hermes` or `ansatz chat`
...
## Using `ansatz -w` (Automatic Worktree Mode)
...
```bash
ansatz -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `ansatz -w` in each — every invocation gets its own worktree and branch automatically.
