...
[Nous Portal](/user-guide/features/tool-gateway) provides 300+ models under one subscription. On a fresh install, run `ansatz setup --portal` to log in and set Nous as your provider in one command. Inspect what's wired up with `ansatz portal info`.

- Portal subscribers also get **10% off token-billed providers**.
:::

:::note `model:` schema — empty string vs. mapping
On a brand-new install the bundled default config has `model: ""` (an empty string sentinel meaning "not configured yet"). The first time you run `ansatz setup` or `ansatz model`, that key is upgraded in-place to a mapping with `provider`, `default`, `base_url`, and `api_mode` sub-keys — the shape shown throughout this page and in [`profiles.md`](./profiles.md) / [`configuration.md`](./configuration.md). If you ever see an empty string in `config.yaml`, run `ansatz model` (or click **Change** in the dashboard) and Hermes will write the dict form for you.
:::
...
| **Skills Hub** | `ansatz skills search` uses this. Usually fine at `auto`. |
| **MCP** | MCP tool routing. Usually fine at `auto`. |
| **Triage Specifier** | Routes the Kanban triage specifier (`ansatz kanban specify`) that expands a rough one-liner into a concrete spec. A cheap, capable model works well. |
...
| **Profile Describer** | Routes profile-description generation (`ansatz profile describe --auto` / the dashboard auto-generate button). Short, cheap call. |
...
With discovery off, the model picker (`ansatz model`, `/model`) shows the configured list instead of a live probe.
...
:::note Legacy format
Older configs used a top-level `custom_providers:` list (with `base_url` instead of `api`). It still works and is auto-migrated to the `providers:` dict on `ansatz update` (config v12).
:::

## When does it take effect?

- **CLI** (`ansatz chat`): next `ansatz chat` invocation.
- **Gateway** (Telegram, Discord, Slack, etc.): next *new* session. Existing sessions keep their model. Restart the gateway (`ansatz gateway restart`) if you want to force all sessions to pick up the change.
...
Hermes lists a provider only if it has a working credential. Check **Keys** in the sidebar — you should see one of: an API key, a successful OAuth, or a custom endpoint URL. If the provider you want isn't there, run `ansatz setup` to wire it up, or go to **Keys** and add the env var.
...
Inside any `ansatz chat` session:
...
**Short string form (`model.aliases.<name>: provider/model`)** — convenient from the shell because `ansatz config set` only writes scalar values, but it can't carry a custom `base_url`:
...
### `ansatz model` subcommand

```bash
ansatz model            # Interactive provider + model picker (the canonical way to switch defaults)
```

`ansatz model` walks you through picking a provider, authenticating (OAuth flows open a browser; API-key providers prompt for the key), and then choosing a specific model from that provider's curated catalog. The choice is written to `model.provider` and `model.default` in `~/.hermes/config.yaml`.

To list providers/models without launching the picker, use the dashboard or the REST endpoints below. To inspect what the CLI will actually use right now: `ansatz config get model --json` and `ansatz status`.
