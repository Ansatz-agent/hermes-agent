...
The Hermes web dashboard (`ansatz dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:
...
2. Restart `ansatz dashboard` — plugin API routes are mounted once at startup, **not** on rescan.
