...
```bash
ansatz kanban init           # optional; first `ansatz kanban <anything>` auto-inits
ansatz dashboard             # opens http://127.0.0.1:9119 in your browser
...
- `ansatz kanban --help` — every subcommand, every flag.
- `ansatz kanban watch --kinds completed,gave_up,timed_out` — live stream terminal events across the whole board.
- `ansatz kanban notify-subscribe <task> --platform telegram --chat-id <id>` — get a gateway ping when a specific task finishes.
