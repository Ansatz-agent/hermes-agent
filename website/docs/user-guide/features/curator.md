...
On a brand-new install (or the first time a pre-curator install ticks after `ansatz update`), the curator **does not run immediately**. The first observation seeds `last_run_at` to "now" and defers the first real pass by one full `interval_hours`. This gives you a full interval to review your skill library, pin anything important, or opt out entirely before the curator ever touches it.

If you want to see what the curator *would* do before it runs for real, run `ansatz curator run --dry-run` — it produces the same review report without mutating the library.
:::
...
**Easiest — `ansatz model`:**

```bash
ansatz model                   # → "Auxiliary models — side-task routing"
...
:::note Legacy config
Earlier releases used a one-off `curator.auxiliary.{provider,model}` block. That path still works but emits a deprecation log line — please migrate to `auxiliary.curator` above so the curator shares the same plumbing (`ansatz model`, dashboard Models tab, `base_url`, `api_key`, `timeout`, `extra_body`) as every other aux task.
:::

## CLI

```bash
ansatz curator status         # last run, counts, pinned list, LRU top 5
ansatz curator run            # trigger a run now (blocks until done). Prune-only unless curator.consolidate: true
ansatz curator run --consolidate # force the LLM consolidation pass on for this run, overriding the config default
ansatz curator run --background  # fire-and-forget: start the run in a background thread
ansatz curator run --dry-run  # preview only — report without any mutations
ansatz curator backup         # take a manual snapshot of ~/.hermes/skills/
ansatz curator rollback       # restore from the newest snapshot
ansatz curator rollback --list     # list available snapshots
ansatz curator rollback --id <ts>  # restore a specific snapshot
ansatz curator rollback -y         # skip the confirmation prompt
ansatz curator pause          # stop runs until resumed
ansatz curator resume
ansatz curator pin <skill>    # never auto-transition this skill
ansatz curator unpin <skill>
ansatz curator adopt <skill>    # hand an unmanaged skill to the curator
ansatz curator adopt --all-unmanaged   # hand over every unmanaged skill
ansatz curator list-unmanaged   # itemize skills with no provenance marker
ansatz curator restore <skill>  # move an archived skill back to active
ansatz curator list-archived    # list skills currently in ~/.hermes/skills/.archive/
ansatz curator archive <skill>  # manually archive a single skill now
ansatz curator prune [--days N] # bulk-archive agent-created skills idle >= N days (default 90)
```
...
```bash
ansatz curator rollback        # restore newest snapshot (with confirmation)
ansatz curator rollback -y     # skip the prompt
ansatz curator rollback --list # see all snapshots with reason + size
```
...
You can also take manual snapshots at any time with `ansatz curator backup --reason "before-refactor"`. The `--reason` string lands in the snapshot's `manifest.json` and is shown in `--list`.
...
Set `curator.backup.enabled: false` to disable automatic snapshotting. The manual `ansatz curator backup` command still works when backups are disabled only if you set `enabled: true` first — the flag gates both paths symmetrically so there's no way to accidentally skip the pre-run snapshot on mutating runs.

`ansatz curator status` also lists the five least-recently-used skills — a quick way to see what's likely to become stale next.
...
**To see which skills the curator actually manages**, run `ansatz curator status`.
...
`ansatz curator status` reports an **unmanaged** count alongside the managed
one:
...
unmanaged (no provenance marker): 112 total
  ...
  never auto-staled or archived — `ansatz curator adopt <name>` hands one over
```
...
```bash
ansatz curator list-unmanaged                    # itemize them, with reasons
ansatz curator adopt <name> [<name> ...]         # hand specific skills over
ansatz curator adopt --all-unmanaged --dry-run   # preview the full list
ansatz curator adopt --all-unmanaged             # hand over everything (prompts)
ansatz curator adopt --all-unmanaged --yes       # skip the prompt
```
...
`.usage.json`; read it as policy. `ansatz curator adopt` changes the policy, and
...
- Archives are recoverable via `ansatz curator restore <name>`

If you want to protect a specific skill from ever being touched — for example a
hand-authored skill you rely on — use `ansatz curator pin <name>`. See the next
section.
...
- The **agent's `skill_manage` tool** refuses `delete` on it, pointing the user at `ansatz curator unpin <name>`. Patches and edits still go through, so the agent can improve a pinned skill's content as pitfalls come up without a pin/unpin/re-pin dance.

Pin and unpin with:

```bash
ansatz curator pin <skill>
ansatz curator unpin <skill>
```
...
```bash
ansatz curator restore <skill-name>
```
...
- **For just one run:** `ansatz curator pause` — the pause persists across sessions; use `resume` to re-enable.
