...
| **X Search** | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool — gated on xAI credentials (SuperGrok OAuth or `XAI_API_KEY`); off by default, opt in via `ansatz tools` → 🐦 X (Twitter) Search. |
...
| **Automation** | `cronjob` | Scheduled tasks with create/list/update/pause/resume/run/remove actions. Outbound delivery is handled by cron's own delivery, the `ansatz send` CLI, and the gateway notifier — not by an agent-callable tool. |
...
Paid [Nous Portal](https://portal.nousresearch.com) subscribers can use web search, image generation, TTS, and browser automation through the **[Tool Gateway](tool-gateway.md)** — no separate API keys needed. Run `ansatz model` to enable it, or configure individual tools with `ansatz tools`.
:::
...
ansatz chat --toolsets "web,terminal"
...
# Configure
ansatz config set terminal.backend singularity
ansatz config set terminal.singularity_image ~/python.sif
```
...
modal setup
ansatz config set terminal.backend modal
```
...
ansatz config set terminal.backend vercel_sandbox
ansatz config set terminal.vercel_runtime node24
```
