...
By default every profile is seeded with the bundled skill catalog, and each `ansatz update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:
...
```bash
ansatz profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
ansatz skills opt-out            # stop future seeding — nothing on disk is touched
ansatz skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
ansatz skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `ansatz update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `ansatz skills opt-in`) to re-enable.

:::note Safe by default
`ansatz skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Hermes installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
```bash
ansatz chat --toolsets skills -q "What skills do you have?"
ansatz chat --toolsets skills -q "Show me the axolotl skill"
```
...
When a missing value is encountered, Hermes asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `ansatz setup` or `~/.hermes/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `ansatz config migrate` prompts for unconfigured settings, and `ansatz config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
- `description` (optional) — short text shown in `/bundles` and `ansatz bundles list`.
...
ansatz bundles create research

# Overwrite an existing bundle
ansatz bundles create backend-dev --skill ... --force

# Delete a bundle
ansatz bundles delete backend-dev

# Re-scan ~/.hermes/skill-bundles/ and report changes
ansatz bundles reload
```
...
```bash
ansatz skills browse                              # Browse all hub skills (official first)
ansatz skills browse --source official            # Browse only official optional skills
ansatz skills search kubernetes                   # Search all sources
ansatz skills search react --source skills-sh     # Search the skills.sh directory
ansatz skills search https://mintlify.com/docs --source well-known
ansatz skills inspect openai/skills/k8s           # Preview before installing
ansatz skills install openai/skills/k8s           # Install with security scan
ansatz skills install official/security/1password
ansatz skills install skills-sh/vercel-labs/json-render/json-render-react --force
ansatz skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
ansatz skills list --source hub                   # List hub-installed skills
ansatz skills check                               # Check installed hub skills for upstream updates
ansatz skills update                              # Reinstall hub skills with upstream changes when needed
ansatz skills audit                               # Re-scan all hub skills for security
ansatz skills uninstall k8s                       # Remove a hub skill
ansatz skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
ansatz skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
ansatz skills publish skills/my-skill --to github --repo owner/repo
ansatz skills snapshot export setup.json          # Export skill config
ansatz skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `ansatz skills search <query> --source skills-sh`. Hermes resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
```bash
ansatz skills search https://mintlify.com/docs --source well-known
ansatz skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
ansatz skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```
...
```bash
ansatz skills install openai/skills/k8s
ansatz skills tap add myorg/skills-repo
```
...
```bash
ansatz skills search airbnb --source browse-sh
ansatz skills inspect browse-sh/airbnb.com/search-listings-ddgioa
ansatz skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
```bash
ansatz skills install https://sharethis.chat/SKILL.md
ansatz skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
ansatz skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `ansatz skills update` re-fetches from the same URL automatically when you want to refresh.
...
`ansatz skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
```bash
ansatz skills check          # Report which installed hub skills changed upstream
ansatz skills update         # Reinstall only the skills with updates available
ansatz skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Hermes users add with `ansatz skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
```bash
ansatz skills tap add my-org/hermes-skills
ansatz skills search deploy
ansatz skills install my-org/hermes-skills/deploy-runbook
```
...
The `ansatz skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `ansatz skills tap list` shows the effective path per tap.
...
```bash
ansatz skills install owner/repo/skills/my-workflow
```
...
```bash
ansatz skills tap list                                # show all configured taps
ansatz skills tap add myorg/skills-repo               # add (default path: skills/)
ansatz skills tap remove myorg/skills-repo            # remove
```
...
## Bundled skill updates (`ansatz skills reset`)

Hermes ships with a set of bundled skills in `skills/` inside the repo. On install and on every `ansatz update`, a sync pass copies those into `~/.hermes/skills/` and records a manifest at `~/.hermes/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).
...
`ansatz skills reset` is the escape hatch:
...
ansatz skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `HERMES_HOME`, so `ansatz -p coder skills reset <name>` only affects that profile.
:::
