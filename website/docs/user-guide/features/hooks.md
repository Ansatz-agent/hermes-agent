...
- Runtime event-name validity comes from `hermes_cli.plugins.VALID_HOOKS`. `ansatz hooks list` lists configured shell/outbound hooks, not every available event; `ansatz hooks test <event>` reports the valid set only when an invalid event is supplied.
...
`fail_closed` only applies to blocking-capable events (`pre_tool_call` today); setting it on any other event logs a warning at config-parse time and is ignored. `ansatz hooks test` reflects these semantics — the `parsed` line shows exactly the block shape the dispatcher would receive.
...
1. `--accept-hooks` flag on the CLI (e.g. `ansatz --accept-hooks chat`)
...
**Script edits are silently trusted.** The allowlist keys on the exact command string, not the script's hash, so editing the script on disk does not invalidate consent. `ansatz hooks doctor` flags mtime drift so you can spot edits and decide whether to re-approve.
...
The command string must match the configured hook command exactly. A path-keyed object with a `sha256` field is not the expected format and will not approve the hook. Verify manual entries with `ansatz hooks list`.

### The `ansatz hooks` CLI

| Command | What it does |
|---------|--------------|
| `ansatz hooks list` | Dump configured hooks with matcher, timeout, and consent status |
| `ansatz hooks test <event> [--for-tool X] [--payload-file F]` | Fire every matching hook against a synthetic payload and print the parsed response |
| `ansatz hooks revoke <command>` | Remove every allowlist entry matching `<command>` (takes effect on next restart) |
| `ansatz hooks doctor` | For every configured hook: check exec bit, allowlist status, mtime drift, JSON output validity, and rough execution time |
...
- Re-run `ansatz hooks doctor` after you pull a shared config to spot newly-added hooks before they register.
...
Secrets: prefer `secret_env` (the name of an environment variable, typically set in `~/.hermes/.env`) over an inline `secret:` literal, so the config file stays free of credentials. Entries without a secret are delivered unsigned (flagged as `UNSIGNED` by `ansatz hooks list`).
...
`ansatz hooks list` shows configured outbound targets alongside shell hooks, including whether each target is signed.
