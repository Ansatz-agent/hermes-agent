...
command (`ansatz -p <profile>`) instead of routing.
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `ansatz tools` (Voice section) sets them up.
...
```bash
ansatz config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
ansatz config set wake_word.input_device null
```
