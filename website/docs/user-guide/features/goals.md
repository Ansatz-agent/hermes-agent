...
| Many independent tasks, with dependencies, handoffs, or multiple profiles | [Kanban](./kanban) — `ansatz kanban create …` |
| One card on the board that should keep iterating until its acceptance criteria are met | A kanban card with `--goal` |

:::note
If you want work on the board, put it there yourself (`ansatz kanban create …`) — `/goal` won't do it for you. The reverse is also true: pausing, resuming, or clearing a goal in this chat never creates, claims, or moves a kanban card.
:::
