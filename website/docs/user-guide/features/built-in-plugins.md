...
`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `ansatz memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `ansatz plugins list` and the interactive `ansatz plugins` UI), but none load until you explicitly enable them:

```bash
ansatz plugins enable disk-cleanup
```
...
```bash
ansatz plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `ansatz plugins enable <name>`.
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `ansatz memory` and `ansatz plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
**Enabling:** `ansatz plugins enable disk-cleanup` (or check the box in `ansatz plugins`).

**Disabling again:** `ansatz plugins disable disk-cleanup`.
...
**Enabling:** `ansatz plugins enable security-guidance` (or check the box in `ansatz plugins`).

**Disabling again:** `ansatz plugins disable security-guidance`.
...
```bash
ansatz tools          # → Langfuse Observability → Cloud or Self-Hosted
```
...
ansatz plugins enable observability/langfuse
```
...
Session grouping keys off the Hermes session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `ansatz chat` session lives under one Langfuse session.

**Verify:**

```bash
ansatz plugins list                 # observability/langfuse should show "enabled"
ansatz chat -q "hello"              # check the Langfuse UI for a "Hermes turn" trace
```
...
**Disabling:** `ansatz plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
```bash
ansatz plugins enable observability/nemo_relay
```
...
```bash
ansatz plugins enable google_meet
...
**Disabling:** `ansatz plugins disable google_meet`. Any saved transcripts stay in `~/.hermes/workspace/meetings/` until you remove them.
...
**Enabling:** Nothing to enable — `hermes-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `ansatz dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.
...
- Manifest source is reported as `bundled` in `ansatz plugins list`
