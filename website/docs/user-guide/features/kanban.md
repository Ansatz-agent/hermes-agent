...
ansatz kanban boards list

# Create a new board.
ansatz kanban boards create atm10-server \
    --name "ATM10 Server" \
...
ansatz kanban --board atm10-server list
ansatz kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
ansatz kanban boards switch atm10-server
ansatz kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
ansatz kanban boards rename atm10-server "ATM10 (Prod)"
...
3. `~/.hermes/kanban/current` — the slug persisted by `ansatz kanban
   boards switch`.
4. `default`.
...
`ansatz dashboard` → Kanban tab shows a board switcher at the top as soon
...
ansatz kanban create "research AI funding landscape" --assignee researcher
...
When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `ansatz kanban show t_abcd`.
...
for debugging. Standard gateway supervision applies: run `ansatz gateway
...
until one comes up — `ansatz kanban create` warns about this at creation
time.

Running `ansatz kanban daemon` as a separate process is **deprecated**;
...
ansatz kanban create "nightly ops review" \
    --assignee ops \
...
```bash
ansatz kanban complete t_abc t_def t_hij --result "batch wrap"
ansatz kanban archive  t_abc t_def t_hij
ansatz kanban unblock  t_abc t_def
ansatz kanban block    t_abc "need input" --ids t_def t_hij
```
...
**Workers do not shell out to `ansatz kanban`.** When the dispatcher spawns a worker it sets `HERMES_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `ansatz kanban` CLI.
...
**Zero schema footprint on normal sessions.** A regular `ansatz chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `HERMES_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
```bash
ansatz kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

ansatz kanban create "audit auth flow" \
    --assignee reviewer \
...
The dispatcher emits one `--skills <name>` flag per skill listed, so the worker spawns with all of them loaded on top of the auto-injected kanban guidance. The skill names must match skills that are actually installed on the assignee's profile (run `ansatz skills list` to see what's available); there's no runtime install.
...
```bash
# At creation
ansatz kanban create "hard refactor" --assignee coder \
    --model claude-opus-4.6 --provider anthropic

# Or later — takes effect on the next dispatch
ansatz kanban set-model t_abcd claude-opus-4.6 --provider anthropic
ansatz kanban set-model t_abcd none    # clear the override
```
...
For the occasional quality-sensitive card, pin just that task back to a stronger model with the [per-task model override](#per-task-model-override) (`--model`/`--provider` at create time, `ansatz kanban set-model` later, or the dashboard's model dropdown) — no profile edits needed.
...
```bash
ansatz kanban create "Translate the docs site to French" \
...
```bash
ansatz kanban init      # one-time: create kanban.db if not already present
ansatz dashboard        # "Kanban" tab appears in the nav, after "Skills"
```
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `ansatz kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `ansatz profile describe --auto`). |
...
Tasks in `~/.hermes/kanban.db` are profile-agnostic on purpose (that's the coordination primitive). If you open the dashboard with `ansatz -p <profile> dashboard`, the board still shows tasks created by any other profile on the host. Same user owns all profiles, but this is worth knowing if multiple personas coexist.
...
```
ansatz kanban init                                     # create kanban.db + print daemon hint
ansatz kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
ansatz kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
ansatz kanban show <id> [--json]
ansatz kanban assign <id> <profile>                    # or 'none' to unassign
ansatz kanban reassign <id>... <profile>               # bulk re-assign tasks to a profile
ansatz kanban edit <id> [--title ...] [--body ...]     # edit task title / body / priority in place
        [--priority N]
ansatz kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
ansatz kanban schedule <id> --at <ISO8601>             # set/clear a task's scheduled_at start time
ansatz kanban diagnostics [--json]                     # board health snapshot (alias: diag)
ansatz kanban link <parent_id> <child_id>
ansatz kanban unlink <parent_id> <child_id>
ansatz kanban claim <id> [--ttl SECONDS]
ansatz kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
ansatz kanban complete <id>... [--result "..."]
ansatz kanban block <id> "<reason>" [--ids <id>...]
ansatz kanban unblock <id>...
ansatz kanban archive <id>...

ansatz kanban request-review <id> [--summary "..."] [--metadata JSON] [--reviewer PROFILE]
ansatz kanban request-changes <id> "<required changes>"               # active reviewer -> implementer
ansatz kanban reopen-review  <id>... [--reason "..."]                 # changes requested: 'review' -> ready/todo

ansatz kanban tail <id>                                # follow a single task's event stream
ansatz kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
ansatz kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
ansatz kanban runs <id> [--json]                       # attempt history (one row per run)
ansatz kanban assignees [--json]                       # profiles on disk + per-assignee task counts
ansatz kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
ansatz kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `ansatz gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
ansatz kanban stats [--json]                           # per-status + per-assignee counts
ansatz kanban log <id> [--tail BYTES]                  # worker log from ~/.hermes/kanban/logs/
ansatz kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
        [--chat-type dm|group|channel|thread] [--delivery-mode notify|notify+wake|wake]
ansatz kanban notify-list [<id>] [--json]
ansatz kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
ansatz kanban context <id>                             # what a worker sees
ansatz kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
ansatz kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
ansatz kanban create "nightly backup audit" \
...
Every `ansatz kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `ansatz chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `hermes_cli.kanban.run_slash()` entry point that reuses the `ansatz kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `ansatz kanban`. You don't have to leave the chat to drive the board.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`ansatz kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `ansatz kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |
...
ansatz kanban create "Fix CI failure from t_impl: test_retry flakes on 3.11" \
    --assignee coder \
    --parent t_impl \
    --body "$(cat <<'EOF'
...
```bash
ansatz kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
...
```bash
ansatz kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7 \
    --chat-type group --delivery-mode notify+wake
ansatz kanban notify-list
ansatz kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
ansatz kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
ansatz kanban runs t_abcd
...
**Bulk close caveat.** `ansatz kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`ansatz kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `ansatz kanban heartbeat $TASK` to signal liveness during long operations. |
...
`ansatz kanban tail <id>` shows these for a single task. `ansatz kanban watch` streams them board-wide.
