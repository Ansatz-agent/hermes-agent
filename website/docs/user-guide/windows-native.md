...
No admin rights required. The installer goes to `%LOCALAPPDATA%\hermes\` and adds `ansatz` to your **User PATH** — open a new terminal after it finishes.
...
| `-SkipSetup` | off | Skip the post-install `ansatz setup` wizard |
...
10. **Runs `ansatz setup`** — the normal first-run wizard (model, provider, toolsets). Skip with `-SkipSetup`.
...
|---|---|---|
| CLI (`ansatz chat`, `ansatz setup`, `ansatz gateway`, …) | ✓ | ✓ |
| Interactive TUI (`ansatz --tui`) | ✓ | ✓ |
...
`ansatz gateway install` on Windows uses **Scheduled Tasks** with a Startup-folder fallback — no admin required.

### Install

```powershell
ansatz gateway install
```
...
```powershell
ansatz gateway status      # Merged view: schtasks + Startup folder + running PID
ansatz gateway start       # Starts the scheduled task now
ansatz gateway stop        # Graceful SIGTERM equivalent (TerminateProcess via psutil)
ansatz gateway restart
ansatz gateway uninstall   # Removes schtasks entry, Startup shortcut, pid file
```

`ansatz gateway status` is idempotent — call it a thousand times in a row and it will never accidentally kill the gateway. (Pre-PR #21561 it silently did, via `os.kill(pid, 0)` colliding with `CTRL_C_EVENT` at the C level — see "process management internals" below if you care about the story.)
...
- Playwright Chromium is auto-installed on first run (`npx playwright install chromium`). If installation fails, `ansatz doctor` surfaces it with a fix-it hint.
...
The `ansatz uninstall` CLI subcommand also handles the case where the schtasks entry was registered under a different task name (older installs) — it searches by install path rather than by hardcoded task name.
...
Chromium is auto-installed on first run. If the install failed (rate-limited GitHub, Playwright CDN hiccup), run `ansatz doctor` — it will surface the missing Chromium and print the exact `npx playwright install chromium` command to fix it.
