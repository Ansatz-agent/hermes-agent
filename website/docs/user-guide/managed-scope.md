...
left user-settable. `ansatz doctor` reports the *resolved* managed directory so
...
```bash
ansatz config        # shows a header naming the managed source + the pinned keys
ansatz doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Hermes refuses and names the source:

```bash
$ ansatz config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/hermes/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `ansatz config set` / setup will not write
...
`ansatz doctor` to confirm the policy is being applied).
