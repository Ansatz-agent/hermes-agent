...
ansatz --tui -c
ansatz --tui --continue
ansatz --tui --resume latest

# Resume a specific session by ID or title
ansatz --tui -r 20260409_000000_aa11bb
ansatz --tui --resume "my t0p session"
...
With `display.interface: tui`, a bare `hermes` (and `ansatz chat`) launches the TUI. Explicit flags always win — run `ansatz --cli` to drop back to the classic REPL for a single invocation, or `ansatz --tui` / `HERMES_TUI=1` to force the TUI when the config default is `cli`.
...
- **Node.js** ≥ 20 — the TUI runs as a subprocess launched from the Python CLI. `ansatz doctor` verifies this.
...
Contributors who run `ansatz --tui --dev` from many worktrees can share one `node_modules` instead of installing per checkout — see [TUI & Desktop from Worktrees](../developer-guide/worktree-ui-dev.md).
:::
...
Use the live session switcher when you want one terminal to act as a dispatcher for several TUI sessions. It lists only sessions that are currently live in this TUI process; closed sessions remain saved transcripts and can still be reopened with `/resume` or `ansatz --tui --resume <id-or-title>`.
...
By default, `ansatz --tui` starts a fresh session each launch. To re-attach to the most recent TUI session automatically (useful when your terminal or SSH connection drops unexpectedly), opt in:
...
- **`⚠ YOLO`** — visible warning whenever YOLO mode is on (`ansatz --yolo`, `/yolo`, or `HERMES_YOLO_MODE=1`). The same badge also appears in the startup banner so you cannot launch an auto-approving session without noticing.
...
Launching `hermes` (without `--tui`) stays on the classic CLI by default. To make a machine prefer the TUI, set `display.interface: tui` in `~/.hermes/config.yaml` (persistent) or `HERMES_TUI=1` in your shell profile (per-shell). To go back, set `interface: cli` / unset the env var, or pass `ansatz --cli` for a one-off.
