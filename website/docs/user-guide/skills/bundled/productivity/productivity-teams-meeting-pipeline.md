...
Everything operator-facing is a `ansatz teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
```bash
ansatz teams-pipeline validate              # config snapshot — run first after any change
ansatz teams-pipeline token-health          # Graph token status
ansatz teams-pipeline token-health --force-refresh   # force a fresh token acquisition
ansatz teams-pipeline list                  # recent meeting jobs
ansatz teams-pipeline list --status failed  # only failed jobs
ansatz teams-pipeline show <job-id>         # full detail of one job
ansatz teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
ansatz teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
ansatz teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
ansatz teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
ansatz teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

ansatz teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
ansatz teams-pipeline delete-subscription <sub-id>
ansatz teams-pipeline maintain-subscriptions            # renew near-expiry ones
ansatz teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `ansatz teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `ansatz cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
