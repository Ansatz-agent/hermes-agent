...
- CLI commands: `ansatz --help`, `ansatz <command> --help`, and `hermes_cli/main.py`
...
# Other surfaces
ansatz desktop                 # launch the native desktop app (alias: ansatz gui)
ansatz dashboard               # web admin panel + embedded chat
ansatz proxy                   # OpenAI-compatible local proxy backed by your OAuth provider
```
...
| MCP servers (add, catalog, `ansatz mcp`) | `references/native-mcp.md` |
...
Two theming rules that hold even without loading the reference: **you apply skins yourself** (`ansatz config set display.skin <name>` — every surface repaints live within ~a second; don't tell the user to run `/skin`), and **to tweak one color, edit the ACTIVE skin** (`ansatz skin set <key> <hex>`) — never fork `default`, which drops the palette and resets the background.
...
- **Use `ansatz chat -q` for fire-and-forget** — no PTY needed
...
- **Desktop app** (`ansatz desktop` / `ansatz gui`) — native Electron app for macOS/Linux/Windows: streaming chat, session list, Cmd+K palette, drag-and-drop files, native notifications, per-profile remote-gateway login. Extend it with UI plugins — `references/desktop-plugins.md`.
- **Web dashboard** (`ansatz dashboard`) — full admin panel: messaging channels, MCP catalog, webhooks, memory, profile builder, plus an embedded `ansatz --tui` chat. Secured behind an OAuth/token gate.
- **Ink TUI** (`ansatz --tui` or `display.interface: tui`) — terminal UI with docked widget apps — `references/tui-widgets.md`.
- **OpenAI-compatible proxy** (`ansatz proxy`) — a local OpenAI API backed by whichever OAuth provider you're signed into. Point Codex CLI, Aider, Cline, or any script at it — no API key.
...
- **Never hand-edit `config.yaml` for the user** — use `ansatz config set KEY VAL`; a stray indent can corrupt the file and break the live gateway.
