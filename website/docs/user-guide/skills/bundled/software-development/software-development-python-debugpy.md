...
- A long-running process (ansatz gateway, tui_gateway) misbehaves and you can't restart it
...
### `tui_gateway` subprocess (spawned by `ansatz --tui`)
...
debugpy.wait_for_client()
```
Start `ansatz --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
