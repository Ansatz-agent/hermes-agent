...
| | |
|---|---|
| Source | Optional — install with `ansatz skills install official/mcp/mcp-oauth-remote-gateway` |
...
process (e.g. `ansatz dashboard --host 0.0.0.0 --port <port>`; check with
`ps aux | grep 'ansatz dashboard'`). It exposes a connector/MCP console —
