...
| | |
|---|---|
| Source | Optional — install with `ansatz skills install official/autonomous-ai-agents/honcho` |
...
```bash
ansatz honcho status    # shows resolved config, connection test, peer info
```
...
Manual override: `ansatz honcho map my-project-name`
...
```bash
ansatz honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Run `ansatz honcho setup`. Ensure `memory.provider: honcho` is in `~/.hermes/config.yaml`.

### Memory not persisting across sessions
Check `ansatz honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `ansatz profile create <name> --clone`. For existing profiles: `ansatz honcho sync`.
...
| Command | Description |
|---------|-------------|
| `ansatz honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `ansatz honcho status` | Show resolved config, connection test, peer info for active profile |
| `ansatz honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `ansatz honcho disable` | Disable Honcho for the active profile |
| `ansatz honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `ansatz honcho peers` | Show peer identities across all profiles |
| `ansatz honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `ansatz honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `ansatz honcho sessions` | List known directory-to-session-name mappings |
| `ansatz honcho map <name>` | Map current working directory to a Honcho session name |
| `ansatz honcho identity` | Seed AI peer identity or show both peer representations |
| `ansatz honcho sync` | Create host blocks for all Hermes profiles that don't have one yet |
| `ansatz honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Hermes + Honcho |
| `ansatz memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `ansatz memory status` | Show active memory provider and config |
| `ansatz memory off` | Disable external memory provider |
