...
| | |
|---|---|
| Source | Optional — install with `ansatz skills install official/migration/openclaw-migration` |
...
```bash
ansatz claw migrate              # Full interactive migration
...
ansatz claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `ansatz setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.
