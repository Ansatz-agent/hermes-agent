...
| | |
|---|---|
| Source | Optional — install with `ansatz skills install official/devops/hermes-s6-container-supervision` |
...
│   └── dashboard/run                  ← if HERMES_DASHBOARD=1, runs `ansatz dashboard`
│
├── /run/service (s6-svscan watches; tmpfs)
│   ├── gateway-coder/                 ← runtime-registered per-profile
│   │   ├── type        ("longrun")
│   │   ├── run         ("#!/command/with-contenv sh ... exec s6-setuidgid hermes ansatz -p coder gateway run")
...
| `hermes_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `ansatz gateway start/stop/restart` and routes to s6 when running in a container. |
...
Most likely the profile has no model or auth configured. The service slot is correct — the gateway itself is unconfigured. Run `ansatz -p <profile> setup` first. The s6 supervisor will keep restarting it; that's the desired behavior (when you fix the config, the next attempt succeeds and stays up).

### Reconciler skipped a profile

The reconciler keys on the **presence of `SOUL.md`** as the "real profile" marker. `ansatz profile create` always seeds it. If a profile dir is missing SOUL.md (stray directory, partial restore, backup-in-progress), the reconciler skips it intentionally. Add a `SOUL.md` (even empty) to opt back in.
