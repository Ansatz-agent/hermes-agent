...
| | |
|---|---|
| Source | Optional — install with `ansatz skills install official/payments/stripe-link-cli` |
...
```
ansatz mcp add stripe-link --command "npx" --args "@stripe/link-cli --mcp"
```

Then `ansatz mcp list` should show `stripe-link`. The same approval rules apply — MCP doesn't bypass the Link app approval step.
