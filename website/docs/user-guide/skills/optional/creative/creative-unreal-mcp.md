...
| | |
|---|---|
| Source | Optional — install with `ansatz skills install official/creative/unreal-mcp` |
...
### One-time, Hermes side
    ...
    ansatz mcp install unreal-engine
...
then happens at `ansatz mcp install`/`configure` time instead. Tool-search
