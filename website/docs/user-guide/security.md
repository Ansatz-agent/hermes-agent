...
1. **CLI flag**: Start a session with `ansatz --yolo` or `ansatz chat --yolo`
...
:::tip
Use `ansatz config edit` to review or remove patterns from your permanent allowlist.
:::

### Mining Approval History (`ansatz approvals suggest`)
...
```bash
ansatz approvals suggest            # dry run — prints a numbered proposal
ansatz approvals suggest --apply 1,3  # merge picks into command_allowlist
ansatz approvals suggest --json     # machine-readable output
```
...
Do not ask the agent to `patch` `~/.hermes/cron/jobs.json` directly. Use the `cronjob` tool, [`ansatz cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other Hermes control files when write safety blocks direct edits.
...
3. The bot owner runs `ansatz pairing approve <platform> <code>` on the CLI
...
ansatz pairing approve telegram ABC12DEF

# Revoke a user's access
ansatz pairing revoke telegram 123456789

# Clear all pending codes
ansatz pairing clear-pending
```
...
```bash
docker exec -u hermes hermes-agent ansatz pairing approve telegram ABC12DEF
```
...
10. **Keep updated** — run `ansatz update` regularly for security patches
...
- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `ansatz doctor` for the full remediation.
- **`ansatz doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `ansatz tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `ansatz tools`.
