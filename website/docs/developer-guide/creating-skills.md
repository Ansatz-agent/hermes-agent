...
- `prompt` (optional) — prompt text shown during `ansatz config migrate`; falls back to `description`
...
2. **Discovery:** `ansatz config migrate` scans all enabled skills, finds unconfigured settings, and prompts the user. Settings also appear in `ansatz config show` under "Skill Settings."
...
4. **Manual setup:** Users can also set values directly:
   ```bash
   ansatz config set skills.config.myplugin.path ~/my-data
   ```
...
```bash
ansatz chat --toolsets skills -q "Use the X skill to do Y"
```
...
If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo, is discoverable via `ansatz skills browse` (labeled "official"), and installs with built-in trust.

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a registry and share it via `ansatz skills install`.
...
Because a blueprint **is** a skill, it flows through the entire skills pipeline unchanged — search, inspect, install, security scan, provenance, taps, the centralized index, and `ansatz skills publish` for sharing. Nothing new to learn.
...
```bash
ansatz skills install owner/morning-brief
...
**Sharing an automation you built.** A blueprint loaded by a cron job (`ansatz cron create --skill <name> ...`) can be exported back to a SKILL.md and published like any other skill, so an automation you tuned for yourself becomes a one-command install for someone else.
...
```bash
ansatz skills tap add owner/repo
```
