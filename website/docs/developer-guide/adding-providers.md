...
- setup / `ansatz model` menu entries
...
:::tip
`hermes_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `ansatz setup`.
:::
...
7. `ansatz model` menu includes the provider
8. `ansatz setup` wizard delegates to `main.py` automatically
...
- Provider-specific `ansatz model` menu entries with bespoke auth flows
...
A provider is not discoverable until it shows up in the interactive `ansatz model` flow.
...
:::tip
`hermes_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `ansatz model` and `ansatz setup` automatically.
:::
...
### 7. Updating `ansatz model` but not `ansatz setup`
