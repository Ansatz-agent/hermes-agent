...
|---|---|---|
| I just want Hermes working on my machine | `ansatz setup` | Run a real chat and verify it responds |
| I already know my provider | `ansatz model` | Save the config, then start chatting |
| I want a bot or always-on setup | `ansatz gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `ansatz model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `ansatz model` first | Add routing and fallback only after the base chat works |
...
The single most important setup step. Use `ansatz model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `ansatz setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `ansatz update`. Re-enable anything later with `ansatz tools`, seed skills with `ansatz skills opt-in --sync`, or tune settings with `ansatz setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `ansatz model` |
| **OpenAI Codex** | ChatGPT or Codex subscription, uses Codex models | Device code auth via `ansatz model` → **ChatGPT or Codex Subscription** |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `ansatz model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `hermes_cli/models.py` may change between releases) | `ansatz model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `ansatz model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `ansatz model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `ansatz model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `ansatz model` (requires `copilot` CLI + `copilot login`) |
...
:::tip
You can switch providers at any time with `ansatz model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::
...
```bash
ansatz config set model anthropic/claude-opus-4.6
ansatz config set terminal.backend docker
ansatz config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
hermes            # classic CLI
ansatz --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Hermes ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `hermes` vs `ansatz --tui`.
:::
...
```bash
ansatz --continue    # Resume the most recent session
ansatz -c            # Short form
```
...
```bash
ansatz gateway setup    # Interactive platform configuration
```
...
- `ansatz tools` — tune tool access per platform
- `ansatz skills` — browse and install reusable workflows
...
```bash
ansatz config set terminal.backend docker    # Docker isolation
ansatz config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `ansatz egress setup && ansatz egress start`; `ansatz setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.
...
```bash
ansatz skills browse                      # list everything available
ansatz skills search kubernetes           # find skills by keyword
ansatz skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `ansatz skills browse` shows the exact slugs to use.
...
|---|---|---|
| Hermes opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `ansatz model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `ansatz gateway setup` and check `ansatz gateway status` |
| `ansatz --continue` can't find old session | Switched profiles or session never saved | Check `ansatz sessions list` and confirm you're in the right profile |
...
| `ansatz doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `ansatz model`
3. `ansatz setup`
4. `ansatz sessions list`
5. `ansatz --continue`
6. `ansatz gateway status`
...
| `ansatz tools` | Configure which tools are enabled per platform |
| `ansatz setup` | Full setup wizard (configures everything at once) |
| `ansatz doctor` | Diagnose issues |
| `ansatz update` | Update to latest version |
| `ansatz gateway` | Start the messaging gateway |
| `ansatz --continue` | Resume last session |
...
- **[Moving to another machine](/reference/faq#exporting-hermes-to-another-machine)** — `ansatz backup` migrates your whole setup (or [a single profile](/reference/faq#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch
