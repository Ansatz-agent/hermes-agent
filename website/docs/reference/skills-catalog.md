...
Hermes also syncs bundled skills on `ansatz update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.hermes/skills/` tree, it is still shipped with Hermes; restore it with `ansatz skills reset <name> --restore`.
...
| [`packaging-hermes-macos-arm64-dmg`](/docs/user-guide/skills/bundled/software-development/software-development-packaging-hermes-macos-arm64-dmg) | Build and verify Hermes macOS ARM64 DMG installers. | `software-development/packaging-hermes-macos-arm64-dmg` |
| [`packaging-hermes-windows-installer`](/docs/user-guide/skills/bundled/software-development/software-development-packaging-hermes-windows-installer) | Build and verify Windows Setup installers from any host. | `software-development/packaging-hermes-windows-installer` |
