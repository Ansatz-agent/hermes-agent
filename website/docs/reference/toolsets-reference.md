...
```bash
ansatz chat --toolsets web,file,terminal
ansatz chat --toolsets debugging        # composite — expands to file + terminal + web
ansatz chat --toolsets all              # everything
```
...
```bash
ansatz tools                            # curses UI to enable/disable per platform
```
...
| `x_search` | `x_search` | Read-only public X discovery via xAI's built-in `x_search` Responses tool. Use the `xurl` skill for authenticated X API reads and account actions. Off by default; opt in via `ansatz tools`. Schema only registered when xAI credentials (SuperGrok OAuth or `XAI_API_KEY`) are configured. |
...
## Relationship to `ansatz tools`

The `ansatz tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
