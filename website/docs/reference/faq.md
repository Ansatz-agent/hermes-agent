...
- **Anthropic** — Claude models (direct API, OAuth via `ansatz auth add anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `ansatz model` or by editing `~/.hermes/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
Yes. Run `ansatz model`, select **Custom endpoint**, and enter your server's URL:
...
**Solution:** Exit your session and use `ansatz model` from your terminal to add new providers:
...
After adding a new provider via `ansatz model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `ansatz model` (from terminal) |
| Enter/change API keys | `ansatz model` (from terminal) |
...
# Or set directly
ansatz config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```
...
# Set a valid model
ansatz config set HERMES_MODEL anthropic/claude-opus-4.7

# Or specify per-session
ansatz chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `ansatz chat --provider <alternative>` to route to a different backend
...
ansatz chat --model openrouter/google/gemini-3-flash-preview
```
...
- Or switch to the terminal interface for administrative tasks: `ansatz chat`
...
**Solution:**
- Verify your bot token is valid with `ansatz gateway setup`
...
#### WSL: Gateway keeps disconnecting or `ansatz gateway start` fails
...
tmux new -s hermes 'ansatz gateway run'
# Reattach later: tmux attach -t hermes

# Option 3: Background via nohup
nohup ansatz gateway run > ~/.hermes/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'ansatz gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `ansatz gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
ansatz gateway install    # Re-snapshots your current PATH
ansatz gateway start      # Detects the updated plist and reloads
```
...
**Solution:**
- Try a faster/smaller model: `ansatz chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `ansatz chat -t "terminal"`
...
If the baseline looks high before you've typed anything, that's the fixed prompt budget — the system prompt plus tool schemas sent on every call. Run [`ansatz prompt-size`](/reference/cli-commands#hermes-prompt-size) to measure it, then trim: disable toolsets you don't use (`ansatz tools`) and uninstall or disable skills you don't need (`ansatz skills`).
...
ansatz config show | grep -A 12 mcp_servers
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `ansatz profile create newname --clone-all` to copy everything from the current profile, or add `--clone-from <profile>` to copy from a specific source profile.
...
### What happens when I run `ansatz update`?

`ansatz update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `ansatz update` once — it covers every profile on the machine.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `ansatz skills config` settings don't seem to take effect.

**Solution:** Use `ansatz skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`ansatz gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
3. Copy the zip to the new machine and import it:
   ...
   # On the new machine
   ansatz import ~/hermes-backup-<timestamp>.zip
   ```

4. On the new machine, run `ansatz setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
ansatz profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
ansatz profile import ./work-backup.tar.gz work
```
...
### `ansatz backup` vs `ansatz profile export`

| Feature | `ansatz backup` | `ansatz profile export` |
| :--- | :--- | :--- |
...
:::tip
`ansatz backup` produces a consistent snapshot even while Hermes is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::
...
ansatz config show | head -20

# Re-run model selection
ansatz model

# Or test with a known-good model
ansatz chat -q "hello" --model anthropic/claude-opus-4.7
```
...
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Hermes version (`ansatz --version`), and the full error message
