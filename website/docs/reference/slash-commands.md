...
| `/object_context monitor` (aliases: `/object-context`, `/oc`) | **CLI only.** Open a private, self-contained all-session workspace with global KPIs, a searchable run table, and 16 request/turn charts for Object Context token savings, provider-reported prompt-cache hits, provider API latency, and provider token spend. Every user-visible session is listed and tagged `OC` or `No OC`; the left sidebar groups sessions by that persisted projection-evidence tag while search filters both groups. Sessions without Object Context evidence show zero-valued savings while their normal request/turn token, cache, and timing data remain visible. New request events are persisted content-free in SessionDB; old numeric series are recovered from redacted logs when available, otherwise exact aggregate totals remain visible with explicit partial-series coverage instead of fabricated points. Stored session titles are the primary labels and stable IDs are secondary. Savings switches between absolute tokens and relative percentage; cache switches between hit rate and cache-read tokens, with cumulative rates weighted by prompt tokens. Every chart downloads its full displayed series, while the session-wide CSV includes every mode. The current or resumed conversation is selected by default and works before its first new model turn; re-run to refresh. |
