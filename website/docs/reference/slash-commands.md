...
| `/object_context monitor` (aliases: `/object-context`, `/oc`) | **CLI only.** Open a private, self-contained all-session Object Context workspace with global KPIs, a searchable run table, and 16 request/turn charts for tokens saved, provider-reported prompt-cache hits, local projection time, and rendered-context tokens spent. Stored session titles are the primary labels and stable IDs are secondary. The savings charts switch between absolute tokens and relative savings percentage; cache charts switch between cache-hit rate and cache-read tokens, with cumulative rates weighted by prompt tokens. Every chart downloads the currently displayed full series, while the session-wide CSV includes every mode. The current or resumed conversation is selected by default and works before its first new model turn; re-run to refresh the snapshot. |
