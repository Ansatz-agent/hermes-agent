...
| `/object_context monitor` (aliases: `/object-context`, `/oc`) | **CLI only.** Open a private, self-contained all-session Object Context workspace with global KPIs, a searchable run table, and per-project, cumulative-project, per-turn, and cumulative-turn charts for tokens saved, local projection time, and rendered-context tokens spent. Stored session titles are the primary labels and stable IDs are secondary. Select any session to switch its 12 charts. One saved-token control switches all four savings charts between absolute tokens and relative savings percentage; every chart downloads the currently displayed full series, while the session-wide CSV includes both savings modes plus all other chart data. The current or resumed conversation is selected by default and works before its first new model turn; re-run to refresh the snapshot. |
