...
| `/object_context [status\|stats\|on\|off\|set <key> <value>\|reset [key\|all]\|help]` (aliases: `/object-context`, `/oc`) | **CLI only.** Inspect, enable, disable, or configure Object Context V1.1/V1.2 in the active profile. `scheduler` is strictly either `economic` (compatible V1.1 default) or `amortized_batch` (V1.2). V1.2 accepts Hot Tail/W-Q settings plus `batch_policy: dynamic\|fixed` and positive `fixed_batch_size`; fixed counts eligible Pending Deltas, while capacity/emergency remain labelled safety overrides. Pending limits stay fixed at 2x the Hot Tail limits. Status reports the effective runtime scheduler, batch policy, and effective/derived caps. `stats` directly shows estimated latest and cumulative request-projection token savings, retrieval volume, and Working Memory size without an LLM call. Values are strictly validated and persisted to `config.yaml`; changes take effect after restarting the CLI so a live conversation's Context Engine, tool schema, and prompt-cache prefix are never swapped mid-session. |
...
| `/export [profile] [-o out.tar.gz]` | **CLI only.** Pack a profile into a shareable `.tar.gz` — skills, memory, persona, crons, plugins, settings, and (from the desktop) themes and layout. Credentials (`auth.json`, `.env`) are stripped. Defaults to the active profile and `<name>.tar.gz` in the current directory. Same archive as `ansatz profile export`; for a versioned, updatable share use a [profile distribution](../user-guide/profile-distributions.md) instead. |
...
| `/statusbar` (alias: `/sb`) | Toggle the context/model status bar on or off. When Object Context V1 is active, medium/wide layouts show `V1↓0` before any tokens are avoided, then compact latest savings (`V1↓80K`); wide layouts add the reduction percentage plus active-conversation cumulative savings (`Σ↓720K`). |
| `/object_context monitor` (aliases: `/object-context`, `/oc`) | **CLI only.** Open a private, self-contained all-session workspace with global KPIs, a searchable run table, and 36 charts. The historical savings/cache/latency/spend charts and six-chart V1.1 immediate-economic group retain their original meaning. An independent 13-chart V1.2 group shows Hot Tail and Pending state, projected waiting loss `W`, shared rewrite cost `Q`, signed crossing margin, capacity/crossing flags, and the dedicated V1.1 summary-free full-render immediate result as counterfactual comparison telemetry. It uses V1.1 route pricing and never falls back to the V1.2 fixed-policy net. `W` and `Q` are per-decision threshold state, not realized token benefit, and are never summed into savings. Every user-visible session is listed and tagged `OC` or `No OC`; a durable wait-only V1.2 decision counts as OC evidence even before the first Card projection, while legacy economic fields remain unavailable rather than being backfilled. V1.2 decisions remain scheduler/version-labelled, its V1.1 immediate score is comparison telemetry rather than a flush trigger, and historical V1.1 records are never relabelled. New request, decision, and successful-request observation events are persisted content-free. Stored session titles are the primary labels and stable IDs are secondary. Savings switches between absolute tokens and relative percentage; cache switches between hit rate and cache-read tokens, with cumulative rates weighted by prompt tokens. Every chart downloads its full displayed series, while the session-wide CSV includes every mode. The current or resumed conversation is selected by default and works before its first new model turn. The CLI opens an unguessable loopback-only live URL; refreshing that browser page reloads current persisted telemetry. A mode-0600 standalone HTML snapshot remains available as a fallback if the live server cannot start. |
...
```bash
ansatz config set model.aliases.fav anthropic/claude-opus-4.6
ansatz config set model.aliases.grok x-ai/grok-4
```
...
- `/skin`, `/snapshot`, `/export`, `/import`, `/reload`, `/tools`, `/toolsets`, `/browser`, `/config`, `/object_context`, `/cron`, `/platforms`, `/paste`, `/image`, `/statusbar`, `/battery`, `/focus`, `/plugins`, `/busy`, `/indicator`, `/wake`, `/journey`, `/redraw`, `/clear`, `/history`, `/save`, `/copy`, `/handoff`, `/prompt`, `/pet`, `/hatch`, `/timestamps`, `/subscription`, and `/quit` are **CLI-only** commands.
...
- In the TUI, `/sessions` shows live sessions in the current TUI process. Use `/resume [name]` or `ansatz --tui --resume <id-or-title>` for saved or closed transcripts.
