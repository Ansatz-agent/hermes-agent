...
Top-level command for managing profiles. Running `ansatz profile` without a subcommand shows help.
...
ansatz profile use default
```

## `ansatz profile create`

```bash
ansatz profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `ansatz profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `ansatz update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.hermes`), use `ansatz skills opt-out` / `ansatz skills opt-in`. |
...
ansatz profile create mybot

# Clone config only from current profile
ansatz profile create work --clone

# Clone everything from current profile
ansatz profile create backup --clone-all

# Clone config from a specific profile
ansatz profile create work2 --clone-from work

# Clone everything from a specific profile
ansatz profile create work2-backup --clone-from work --clone-all
```

## `ansatz profile describe`

```bash
ansatz profile describe [<name>] [options]
```
...
ansatz profile describe researcher

# Set it explicitly
ansatz profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
ansatz profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
ansatz profile describe --all --auto
```

## `ansatz profile delete`

```bash
ansatz profile delete <name> [options]
```
...
```bash
ansatz profile delete mybot
ansatz profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.hermes`) cannot be deleted — use `ansatz uninstall` to remove everything.
:::

## `ansatz profile show`

```bash
ansatz profile show <name>
```
...
## `ansatz profile alias`

```bash
ansatz profile alias <name> [options]
```
...
```bash
ansatz profile alias work
# Creates/updates ~/.local/bin/work

ansatz profile alias work --name mywork
# Creates ~/.local/bin/mywork

ansatz profile alias work --remove
# Removes the wrapper script
```

## `ansatz profile rename`

```bash
ansatz profile rename <old-name> <new-name>
```
...
```bash
ansatz profile rename mybot assistant
# ~/.hermes/profiles/mybot → ~/.hermes/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `ansatz profile export`

```bash
ansatz profile export <name> [options]
```
...
```bash
ansatz profile export work
# Creates work.tar.gz in the current directory

ansatz profile export work -o ./work-2026-03-29.tar.gz
```
...
## `ansatz profile import`

```bash
ansatz profile import <archive> [options]
```
...
```bash
ansatz profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

ansatz profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `ansatz -p telemetry`). |
...
ansatz profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
ansatz profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
ansatz profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
ansatz profile install ./telemetry/
```

### `ansatz profile update`

```bash
ansatz profile update <name> [--force-config] [--yes]
```
...
`ansatz profile list` also shows the distribution name and version in a
`Distribution` column, and `ansatz profile show <name>` / `delete <name>`
...
ansatz profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
ansatz profile install https://github.com/your-org/internal-assistant.git
```
...
3. Tell recipients to run `ansatz profile install <your-repo-url>`.
...
## `ansatz -p` / `ansatz --profile`

```bash
ansatz -p <name> <command> [options]
ansatz --profile <name> <command> [options]
```
...
```bash
ansatz -p work chat -q "Check the server status"
ansatz --profile dev gateway start
ansatz -p personal skills list
ansatz -p work config edit
```

## `ansatz completion`

```bash
ansatz completion <shell>
```
...
- `ansatz profile <TAB>` — subcommands (list, use, create, etc.)
- `ansatz profile use <TAB>` — profile names
- `ansatz -p <TAB>` — profile names
