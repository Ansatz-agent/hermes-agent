...
| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `computer_use` | Background desktop control via cua-driver — screenshots (SOM / vision / AX), click / drag / scroll / type / key / wait, list_apps, focus_app. Does NOT steal the user's cursor or keyboard focus. Works with any tool-capable model. macOS, Windows, and Linux. | `cua-driver` on `$PATH` (install via `ansatz tools`). |
...
Opt-in toolset (not loaded in the default `hermes-cli` set). Add via `--toolsets video_gen` or enable it in `ansatz tools` → Video Generation, which also walks you through picking a backend.
...
Registered by the bundled `spotify` plugin. Requires an OAuth token — run `ansatz auth spotify` once to authorize.
