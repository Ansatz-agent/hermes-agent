...
- **Measure and trim the fixed prompt** — run `ansatz prompt-size` for a byte breakdown of the system prompt and tool schemas, then disable unused toolsets with `ansatz tools` and uninstall skills you don't need with `ansatz skills`.
