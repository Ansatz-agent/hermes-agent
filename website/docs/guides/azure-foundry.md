...
```bash
az login
ansatz model   # pick Azure Foundry → Entra ID
...
`ansatz doctor` runs a 10 s probe against `DefaultAzureCredential` when `model.auth_mode: entra_id`, reporting which inner credential won (env vars present, managed identity endpoint reachable, etc.).

`ansatz auth` shows a structured status block:
...
| `AZURE_FOUNDRY_BASE_URL` | Endpoint URL (set via `ansatz model`; env var is used as a fallback) |
...
The Azure SDK reads the `AZURE_*` env vars directly. Hermes never inspects them other than to report which sources are present in `ansatz doctor` output.
...
Run `ansatz model` again and the wizard will re-probe. If the probe still picks the wrong mode, you can edit `config.yaml` directly:
