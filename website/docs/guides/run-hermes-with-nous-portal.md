...
ansatz setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: device-code login (works from Cloud Shell, Codespaces, EC2 Instance Connect)
ansatz auth add nous --type oauth
# Then re-run `ansatz setup --portal` to wire the provider + gateway
```
...
`ansatz setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
These rows appear in `ansatz tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, Hermes runs the Portal login inline (without changing your inference provider or your other tools).
...
### `ansatz portal info` shows "not logged in" after `ansatz setup --portal`
...
Re-verify with `ansatz portal info`.
...
`ansatz portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Hermes profiles where you're using the wrong one (check `ansatz profile list`)

### Want to revoke and start clean

```bash
ansatz auth logout nous       # wipes the local refresh token
