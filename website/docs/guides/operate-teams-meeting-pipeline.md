...
```bash
ansatz teams-pipeline validate
```
...
```bash
ansatz teams-pipeline token-health
ansatz teams-pipeline token-health --force-refresh
```
...
```bash
ansatz teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
ansatz teams-pipeline maintain-subscriptions
ansatz teams-pipeline maintain-subscriptions --dry-run
```
...
#!/usr/bin/env bash
exec ansatz teams-pipeline maintain-subscriptions
EOF
...
ExecStart=/usr/local/bin/ansatz teams-pipeline maintain-subscriptions
```
...
```cron
0 */12 * * * /usr/local/bin/ansatz teams-pipeline maintain-subscriptions >> /var/log/hermes/teams-pipeline-maintain.log 2>&1
```
...
```bash
ansatz teams-pipeline subscriptions   # should show expirationDateTime advanced
ansatz teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
ansatz teams-pipeline list
ansatz teams-pipeline list --status failed
ansatz teams-pipeline show <job-id>
```

### Replay a stored job

```bash
ansatz teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
ansatz teams-pipeline fetch --meeting-id <meeting-id>
ansatz teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `ansatz teams-pipeline maintain-subscriptions --dry-run`
- inspect `ansatz teams-pipeline list --status failed`
...
- run `ansatz teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `ansatz teams-pipeline run`
...
- [ ] `ansatz teams-pipeline validate` returns an OK snapshot
- [ ] `ansatz teams-pipeline token-health --force-refresh` succeeds
