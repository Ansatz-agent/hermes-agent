...
```bash
ansatz auth add minimax-oauth --no-browser
```
...
```bash
ansatz config set model.default MiniMax-M2.7
ansatz config set model.provider minimax-oauth
```
...
```bash
ansatz --provider minimax-oauth    # canonical
ansatz --provider minimax-portal   # alias
ansatz --provider minimax-global   # alias
ansatz --provider minimax_oauth    # alias (underscore form)
```
...
To use `minimax-oauth` as the active provider, set `model.provider: minimax-oauth` in `config.yaml` (use `ansatz setup` for the guided flow), or pass `--provider minimax-oauth` for a single invocation:

```bash
ansatz --provider minimax-oauth
```
...
**Fix:** run `ansatz auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Hermes raises a timeout error.

**Fix:** re-run `ansatz auth add minimax-oauth` (or `ansatz model`). The flow starts fresh.
...
**Fix:** run `ansatz model` and select MiniMax (OAuth), or run `ansatz auth add minimax-oauth`.
...
```bash
ansatz auth logout minimax-oauth
```
...
- [ansatz doctor](../reference/cli-commands.md)
