...
- Hermes Agent installed and running (`ansatz gateway`)
...
:::note
`ansatz webhook test <name>` only works for **dynamic subscriptions** created with `ansatz webhook subscribe`. It does not read routes from `config.yaml`.
:::
