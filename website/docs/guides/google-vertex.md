...
- **`google-auth`** — installed automatically the first time you select Vertex (lazy install). Run `ansatz setup` to repair a managed install if that fails.
...
Vertex requires the `google/` vendor prefix on model IDs. The `ansatz model` picker offers:
...
`/model` switches among already-configured providers and models; it does not collect new credentials. Configure Vertex with `ansatz model` first.
...
Hermes lazy-installs it the first time you select the Vertex provider. If that fails, run `ansatz setup` to repair the managed install.
