...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`ansatz webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `ansatz webhook subscribe`) |
...
```bash
ansatz webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
ansatz webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
ansatz webhook subscribe alert-triage \
...
```bash
ansatz webhook subscribe auto-port \
  --events "pull_request" \
