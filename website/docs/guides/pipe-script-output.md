...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `ansatz send`."
---

# Pipe Script Output to Messaging Platforms

`ansatz send` is a small, scriptable CLI that pushes a message to any
...
- Piping any tool's output anywhere (`make | ansatz send --to slack:#builds`)
...
`ansatz send` resolves the message body in this order:

1. **Positional argument** — `ansatz send --to telegram "hi"`
2. **`--file PATH`** — `ansatz send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | ansatz send --to telegram`
...
Because `ansatz send` reuses your Hermes config, the same script works on
...
`ansatz send`. If the Python interpreter can't load because the box is
...
## Does `ansatz send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `ansatz send` calls
...
pointing at the gateway; start it with `ansatz gateway start` and retry.
...
"no channels discovered yet", start the gateway once (`ansatz gateway
...
|----------|----------------|---------------------|---------------|----------|
| `ansatz send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`ansatz send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `ansatz send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `ansatz send` shares with cron delivery.
